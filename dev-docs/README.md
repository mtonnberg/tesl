...
The dev shell is the *only* supported environment. It pins Racket 9.2, OCaml +
dune and PostgreSQL, links the `tesl` Racket package at your checkout, and starts
a local PostgreSQL cluster (step 4). Building outside it eventually fails in a way
...
The Racket runtime and the stdlib resolve through `TESL_REPO_ROOT`. If it is unset
or wrong, **the stdlib will not resolve** and you get import failures that look
like missing-module bugs.
...
This is the single command that decides whether the codebase is green. It runs 13
phases: `dune build`, `dune test`, lifted-stdlib snapshots, the embedded-docs sync
check, format, the `tesl validate` sweep, byte-exact `.rkt` snapshot matching, the
Tesl test files, mutation, integration, the Racket suites, the Racket aggregate
suite, and a boot smoke test.
...
- **`dune test` alone is NOT sufficient.** It is phase 2 of 13. It misses the
  example sweep, the byte-exact snapshots, the Racket suites and the aggregate
  suite. Passing `dune test` says nothing about whether the tree is green.

Fast inner loop while iterating: `RKT_SUITES_SKIP=1 ./ci.sh` skips the Racket and
AI suites. Run the full gate before you commit.
...
|---|---|
| `version mismatch, found 8.18` (or any other Racket version) | **Stale `.zo` bytecode — not a regression.** Tesl targets Racket 9.2 and the dev shell ships 9.2. Clear the compiled caches and recompile: `find . -name compiled -type d -prune -exec rm -rf {} +`, plus `tesl clean` in any project directory, then re-run. |
...
| A phase reports `SKIPPED` | An optional dependency is missing (PostgreSQL, racket/raco, python3, MailHog). Never a hard failure — but also not coverage. |
...
that checks manual coherence with no compiler and no Racket needed.
