...
| ~~L2~~ | **CLOSED 2026-07-30.** ~~`auth` is a crypto-free trust root; insecure session pattern~~ | Closed by `completed/response_metadata_and_cookies.md`. The missing primitive shipped: `Http.setSessionCookie` / `Http.clearSessionCookie` / `Http.sessionToken` (`Tesl.Http`, gated by `cookieCap`), a single fixed `__Host-session; HttpOnly; Secure; SameSite=Lax; Path=/` cookie whose writer demands a `JwtToken`, so an unsigned session cookie cannot be expressed. Cookies attach to 2xx only, so no session is minted on an error path. The guidance half is closed too: `lesson76-sessions.tesl` is the one blessed pattern, `lesson06` now says explicitly that its hand-assembled two-cookie MAC is a primitive demonstration and not the session pattern, `manual/best-practices.md` leads with the JWT cookie, and the plaintext `cookies "user" == "admin"` shape is a `SEC001` error. Spec §21.8. **L1 (`establish`) is untouched and stays open.** | spec §21.8; `lesson76`,`lesson06` | ✅ Closed |
