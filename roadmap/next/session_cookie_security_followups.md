...
> **Status:** Next. Findings from an adversarial review of the session/cookie/JWT/renew work
> (`roadmap/completed/response_metadata_and_cookies.md` + the `JWT.renew` addition). The
> cryptographic core held under attack — no Set-Cookie injection, no header/alg confusion, no
> reserved-claim bypass, no cross-request cookie leak, constant-time compare intact. What follows is
> what did NOT hold, ranked. **F1, F2, F3 and F4 were fixed at review time** (see "Already fixed");
> F5–F9 are open and recorded here.

## F5 — LOW→MEDIUM (latent; live once a key is shared): `JWT.renew`'s `iat` check is `real?`
...
## F6 — LOW: the SSE cookie scope is unreachable, and using it hits the F2 leak
...
## F7 — LOW: `kid` publishes a 64-bit function of the signing key in every token
...
## F9 — INFORMATIONAL: SSE 200s carry `Set-Cookie` beside `Access-Control-Allow-Origin: *`
