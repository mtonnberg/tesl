...
**Do not reach for Monaco yet.** It is ~5 MB — five times the entire compiler bundle — and it would
make the page slower than the compiler it is demonstrating. A small hand-rolled highlighter over a
`contenteditable`, or a ~15 KB library like CodeMirror 6's minimal build, is the right size. Measure
before choosing: the artifact-size assertion in the workflow exists precisely so a dependency cannot
quietly triple the page.
