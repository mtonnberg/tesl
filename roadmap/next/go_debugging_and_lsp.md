...
- 2026-08-19: VS Code attach sessions now prefer Go DAP when `tesl-dap` is configured/on PATH or the workspace Go runtime is available via `go run`; `ProcessTarget` discovers project endpoints from `.tesl-stuff/debug.sock` or `debug.port`. Racket remains the launch fallback until generated-Go program packaging is wired.
