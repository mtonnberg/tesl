...
|---|---|---|
| 0. Contract freeze and traceability | Partial | Inventory exists in this roadmap; machine-readable ownership, frozen transcripts, and capability matrices are not complete. |
| 1. Shared Go protocol/compiler-query foundation | Partial | Core framing, compiler queries, positions, LSP session, cancellation, and race/vet coverage exist; differential fixtures, full scheduling, and remaining compiler-backed behavior remain. |
| 2. Go debug emission and ABI | Partial | ABI v1, debug/release emission, function/test checkpoints, SQL/domain instrumentation, and release filtering exist; all checkpoint classes, negative scans, benchmarks, and full generated-project gates remain. |
| 3. Checkpoint engine, values, domains, and control channel | Partial | Breakpoints, stepping, stop-world control, attach transport, value trees, domain providers, and SQL scopes exist; complete value semantics, fault/leak/stress/replay coverage remain. |
| 4. DAP and headless debugger | Partial | Go DAP framing, attach/launch, lifecycle, named-test filtering, headless snapshots, stacks, scopes, variables, source, evaluate, and domain/SQL scopes exist; full attach/headless parity, differential transcripts, and VS Code smoke remain. |
| 5. LSP | Partial | Go LSP entrypoint and broad method/session coverage exist; exact differential behavior, scheduling/soak gates, and packaged-editor validation remain. |
...
- 2026-08-19: focused Go-emitter coverage and all Go runtime/race/vet/build gates pass. The complete `emit_go` group exceeded the 10-minute verification budget without producing a failure, so Phase 4 remains partial pending bounded full-suite execution plus differential and VS Code validation.
...
- 2026-08-19: Added shared fixture `tests/protocol/dap-core.json` and a Go transcript runner covering initialize, conditional/hit breakpoint registration, threads, and disconnect. Assertions normalize to request sequence, command, and success, leaving IDs and implementation-specific bodies out of the oracle; cross-runtime replay is still pending.
