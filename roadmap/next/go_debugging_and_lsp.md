...
- 2026-08-20: Shipped/dev `tesl compile` emits Go modules and resolves a manifest entrypoint when no file is supplied; explicit Racket compilation is no longer reachable through the shipped CLI. Default and explicit-output Go compile smokes pass. Docker packaging uses the Go container template.
