...
- Current checkout inventory: 262 existing tracked `.rkt` files: 35 under `dsl/` (7 debug runtime), 49 under `tesl/`, 3 under `editor/`, 104 under `example/`, 0 under `templates/`, and 71 under `tests/`. The 72 Racket-only test responsibilities and aggregate/benchmark files are retired; Go/OCaml/CLI manifests own those checks. Racket remains an explicit compatibility/oracle backend for the remaining runtime/example/editor overlap until C3.
...
- `tests/` now has no Racket-only test files. The 74 tracked `.tesl` test sources are authoritative for Go test execution; the former 72 Racket-only responsibilities have explicit Go, OCaml, shell/black-box, protocol, benchmark, or reviewed-obsolete replacement entries.
...
|---|---|---|
| 0. Contract freeze and traceability | Complete | Protocol/schema fixtures and the capability matrix are validated by `tests/protocol/check-contracts.sh`. Historical Racket traceability is retained in this completed roadmap and git history. |
...
- 2026-08-20: `tests/protocol/check-contracts.sh` validates capability, ABI, diagnostic, semantic, and transcript-index contracts.
...
- 2026-08-20: Nix and dev-shell wrappers use Go for `tesl run`, `tesl test`, and `tesl debug-inspect`; the shipped CLI rejects `--backend racket`/`TESL_BACKEND=racket` instead of exposing a runtime it no longer packages. Default test and headless inspection pass focused checks.
...
- 2026-08-20: Shipped/dev `tesl emit go` emits Go modules and resolves a manifest entrypoint when no file is supplied; `tesl compile` remains only as an unadvertised compatibility alias. Default emission smokes pass. Docker packaging uses the Go container template.
...
- 2026-08-20: Shipped/dev `tesl build` follows the Go backend and selects local or container Go output from manifest/flags; non-application sources fail explicitly. Go Docker context staging remains daemon-independent.
...
- 2026-08-20: The exhaustive Go corpus gate validates all tracked test, example, and template sources. Historical replacement rows are retained in git history.
...
1. Make each of the 74 tracked `.tesl` tests the sole source and assert Go compile diagnostics, runtime output, status, side effects, and services.
2. Keep each of the 72 migrated responsibilities tied to a named Go runtime/integration test, OCaml compiler test, black-box test, protocol fixture, benchmark, or reviewed-obsolete disposition.
