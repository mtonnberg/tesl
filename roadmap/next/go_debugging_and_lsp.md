...
- 2026-08-20: Added `roadmap/next/go-test-migration.json` and `scripts/check-go-test-inventory.sh`; the gate verifies the measured root inventory (146 Racket tests, 71 paired `.tesl` sources, 75 Racket-only rows) and runs from both the migration-manifest check and Go CLI smoke. The counts are enforced, but the 75 Racket-only rows still need named replacement or reviewed-obsolete dispositions.
- 2026-08-20: Expanded the test migration manifest to all 75 Racket-only root rows with concrete owners, replacement forms, and `planned` status. The validator now rejects missing/extra paths and malformed dispositions; no row is marked green yet.
- 2026-08-20: Added `scripts/run-go-test-manifest.sh` with list, single-source Go compile, and explicit full-corpus compile modes. Focused smoke now validates the 71-source manifest and compiles `tests/units-tests.tesl`; aggregate Racket runner rows remain planned because full runtime/service execution is not yet replaced.
