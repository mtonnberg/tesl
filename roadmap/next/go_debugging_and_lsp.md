...
- 2026-08-20: Expanded the test migration manifest to all 75 Racket-only root rows with concrete owners and replacement forms. 72 rows are now green against Go/runtime evidence; the three OTLP rows remain explicitly planned because Go OTLP export/traces are not implemented.
...
- 2026-08-20: Full paired corpus gate passes: all 71 `.tesl` sources compile and execute as generated Go tests. Full example gate passes for 107 tracked examples. Added Go proof/codec benchmarks and removed stale unpaired example/template `.rkt` artifacts. 72/75 Racket-only rows have Go evidence; the three OTLP rows remain planned because OTLP export/traces are not implemented by the Go runtime.
