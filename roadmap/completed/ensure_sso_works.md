...
**Implementation note folded in 2026-07-31** — the header-trust `auth` discharge was attempted as an *advisory linter warning* (SEC006: flag an `auth`/`establish` block whose `ok <v> ::: Fact` value is tainted by a request field with no verifying `check`, discharged by a verifying head or by `listenAddress Loopback`). Built green, but a full shipped-corpus sweep produced 27 hits across 13 files: it fires on the ordinary, *correct* `establish` idiom (minting a domain-validation fact such as `ValidSearch` from a query parameter) and on the tutorial's own canonical teaching `auth` blocks that read a plaintext cookie — because a linter cannot tell an authentication fact from a validation fact (fact names are conventions, not guarantees). That is exactly the decide-by-spelling / recognisable-shape failure mode this document rejects, and it violates the precision rule (a security lint that fires on clean or canonical code trains people to ignore the whole category; Tesl has no suppression mechanism). The prototype was reverted. This **confirms the sixth review's conclusion**: the discharge must be the runtime-minted `Proxy.verifyBinding` → `ProxyBound` witness (landed) plus topology (`listenAddress Loopback`), a fail-closed error gated on dataflow — **not** an advisory taint heuristic. Full-enforcement header-trust discharge therefore remains **DEFERRED** to the focused whole-module dataflow design (auth-fn → server `listenAddress`); see `SSO_PROGRESS.md`. See **§Item A**, Risks 57/64.


**Implementation note folded in 2026-07-31** — the mixed-mode Password/Machine session witness gate (Open Questions 15/18) was PROTOTYPED and found UNSOUND as scoped, then reverted. The design demanded a `subject ::: LoggedIn subject` witness on the session-minting chokepoint `Http.setSessionCookie`, with `LoggedIn` minted only by check-shaped `Tesl.Auth` combinators fed by an existing verification fact (`PasswordVerified` from `Crypto.checkPassword`, `ProxyBound`, or `Authentic`). It built and fired precisely for the bound case, BUT `Crypto.checkPassword` mints `PasswordVerified` bound to the stored HASH, not to a subject, so `Auth.passwordSession verified "admin"` forged `LoggedIn "admin"` from a verification of the ATTACKER’s own password (privilege escalation, demonstrated). This is exactly the credential→subject binding this document calls the deferred hard half (Risks 46/56/63/64). Shipping it would be worse than the current documented gap — a sound-looking but forgeable session witness — so it was reverted to green. The finding sharpens the fix: the JWT/SSO path is soundly bindable by DERIVING the subject from the verified claims (`sub`), but the fresh-password path needs a typed credential-store lookup that mints a subject-bound fact (`CredentialFor subject`) before `LoggedIn subject` can be minted without escalation. Confirms the decision to defer OQ15/18 to a focused design session; see `SSO_PROGRESS.md`. See **§Login methods**, **§Item A**, Open Questions 15 and 18, Risks 46/56/63/64.
## Original ask
