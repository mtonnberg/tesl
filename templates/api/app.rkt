...
(define-handler
  ...
  (let ([_ (thsl-src! "templates/api/app.tesl" 185 (list (cons 'requestUser *requestUser) (cons 'todoId *todoId)) (lambda () (telemetry-event! "todo.get" #:attributes (["user.id" (raw-value requestUser.id)]))))]) (let ([existing (thsl-src! "templates/api/app.tesl" 186 (list (cons 'requestUser *requestUser) (cons 'todoId *todoId)) (lambda () (let ([tesl_match (select-one (from Todo) (where (==. (entity-field-ref Todo 'id) todoId)))]) (if tesl_match (Something tesl_match) Nothing))) 'existing)]) (thsl-src-control! "templates/api/app.tesl" 187 (list (cons 'existing *existing) (cons 'requestUser *requestUser) (cons 'todoId *todoId)) (lambda () (let ([tesl-case-2 (raw-value existing)]) (cond [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Nothing)) (thsl-src! "templates/api/app.tesl" 189 (list) (lambda () (reject "Todo not found" #:http-code 404)))] [(and (and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Something)) (let ([todo (hash-ref (adt-value-fields *tesl-case-2) 'value)]) (not (tesl-equal? (raw-value todo.ownerId) (raw-value requestUser.id))))) (let ([todo (hash-ref (adt-value-fields *tesl-case-2) 'value)]) (thsl-src! "templates/api/app.tesl" 191 (list (cons 'todo todo)) (lambda () (reject "Todo not owned by request user" #:http-code 403))))] [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Something)) (let ([todo (hash-ref (adt-value-fields *tesl-case-2) 'value)]) (thsl-src! "templates/api/app.tesl" 193 (list (cons 'todo todo)) (lambda () todo)))])))))))
