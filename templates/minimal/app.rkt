...
(define-auther
  ...
  (thsl-src-control! "templates/minimal/app.tesl" 81 (list (cons 'request *request)) (lambda () (let ([tesl-case-0 (raw-value (tesl_import_Http_sessionToken *request))]) (cond [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Nothing)) (thsl-src! "templates/minimal/app.tesl" 82 (list) (lambda () (reject "Missing session cookie" #:http-code 401)))] [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Something)) (let ([token (hash-ref (adt-value-fields *tesl-case-0) 'value)]) (thsl-src! "templates/minimal/app.tesl" 84 (list (cons 'token token)) (lambda () (let/check ([tesl-checked-1 (tesl_import_JWT_verify token (raw-value (requireSecret "SESSION_JWT_SECRET")))]) (let ([claims tesl-checked-1]) (let ([tesl-case-2 (raw-value (tesl_import_Dict_lookup "sub" claims))]) (cond [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Nothing)) (thsl-src! "templates/minimal/app.tesl" 86 (list) (lambda () (reject "Session token has no subject" #:http-code 401)))] [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl-case-2) 'value)]) (thsl-src! "templates/minimal/app.tesl" 88 (list (cons 'userId userId)) (lambda () (let ([tesl-case-3 (raw-value (tesl_import_Dict_lookup "role" claims))]) (cond [(and (adt-value? *tesl-case-3) (eq? (adt-value-variant *tesl-case-3) 'Something)) (let ([role (hash-ref (adt-value-fields *tesl-case-3) 'value)]) (thsl-src! "templates/minimal/app.tesl" 89 (list (cons 'role role)) (lambda () (accept Authenticated #:value (SessionUser #:id *userId #:role *role)))))] [(and (adt-value? *tesl-case-3) (eq? (adt-value-variant *tesl-case-3) 'Nothing)) (thsl-src! "templates/minimal/app.tesl" 90 (list) (lambda () (accept Authenticated #:value (SessionUser #:id *userId #:role "user"))))])))))])))))))])))))
