...
per active ISO 4217 national/territorial currency.  This script bakes:
  ...
  * runtime/go/teslrt/currency_data.go — the same table for the Go runtime, which
                                       needs it for `Currency.fromCode` (the one
                                       lookup that is not decided at compile time).
...
OUT_GO = ROOT / "runtime" / "go" / "teslrt" / "currency_data.go"
...
def main() -> None:
    ...
    # ── runtime/go/teslrt/currency_data.go ──────────────────────────────────
    go = [
        "package teslrt",
        "",
        "// Generated by scripts/gen-currencies.py from the ISO 4217 active list.",
        "// DO NOT EDIT BY HAND — regenerate and commit when ISO 4217 changes.",
        f"// {len(CURRENCIES)} currencies. minorDigits is load-bearing (rounding and",
        "// Money.display): USD=2, JPY=0, BHD=3.",
        "//",
    ...
    for iso, _num, dig in sorted(CURRENCIES):
        go.append(f'\t"{iso}": {{Code: "{iso}", MinorDigits: {dig}}},')
    go += ["}", ""]
    OUT_GO.write_text("\n".join(go))
