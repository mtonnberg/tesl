...
# `-E UTF8`: without it a C-locale initdb creates an SQL_ASCII cluster, which accepts invalid
# byte sequences and compares non-ASCII text differently from the UTF8 database production
# runs against — so parity results obtained here would not transfer.
initdb -D "$TESL_POSTGRES_DATA_DIR" -A trust -U "$TESL_POSTGRES_USER" --locale=C -E UTF8 >/dev/null
