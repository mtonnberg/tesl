...
# Every directory holding committed .rkt snapshots.  `example/chat` and
# `example/kanel` are easy to forget — `example/*.tesl` does not glob into a
# subdirectory, and missing one produces exactly one confusing gate failure
# ("ASSERT chat-backend.tesl: exact output mismatch") long after the sweep looked
# clean.  ci.sh builds its own file list the same way (LEARN_FILES / KANEL_FILES /
# CHAT_FILES / EXAMPLE_TOP_FILES / TEST_FILES), so this list mirrors that one.
DIRS="${*:-example/learn example example/chat example/kanel tests}"
...
for dir in $DIRS; do
  ...
  for tesl in "$REPO_ROOT/$dir"/*.tesl; do
    ...
    tmp="$rkt.regen.$$"
    if "$MAIN" "$tesl" > "$tmp" 2>/dev/null && [ -s "$tmp" ]; then
