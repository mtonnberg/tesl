...
class NetworkTests(unittest.TestCase):
    ...
    def test_negative_control_cannot_be_loopback_or_private(self):
        for address in ("127.0.0.1", "192.168.64.8"):
            with self.subTest(address=address), self.assertRaisesRegex(ValueError, "off-host"):
                network.probe({"remote_address": address})
    ...
    def test_loopback_failures_cannot_be_misreported_as_offline_success(self):
        with patch.object(network, "exchange", side_effect=OSError(errno.EPERM, "loopback denied")):
            with self.assertRaises(OSError):
                network.probe({"remote_address": "140.82.112.4", "address": "192.168.64.8", "token": "test", "tcp": 12, "udp": 13})
    ...
    def test_probe_checks_tcp_udp_ipv4_and_ipv6(self):
        ...
        self.assertEqual([call.args for call in opened.call_args_list],
                         [(family, kind) for kind in (socket.SOCK_STREAM, socket.SOCK_DGRAM)
    ...
    def test_unreachable_positive_control_never_starts_acceptance(self):
        @contextmanager
        def failed_control():
            raise ValueError("cannot establish an off-host positive control")
            yield  # pragma: no cover -- context manager has no reachable value
        with patch.object(network.sys, "platform", "darwin"), \
                patch.object(network, "controls", failed_control), \
                patch.object(network.subprocess, "run") as execute:
            with self.assertRaisesRegex(ValueError, "positive control"):
                network.run(["go", "test"], "/work", {})
        execute.assert_not_called()
