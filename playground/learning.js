...
window.TeslLearning = (() => {
  ...
  const entries = {
    fact: ['fact: give a rule a name', 'A fact names something your program needs to know about particular values. ForCustomer invoice customer means this invoice belongs to this customer. The declaration alone checks nothing: checkCustomer must establish it. Keeping that evidence in the function signature lets the compiler catch callers that skip the check. The check body is trusted code: you still need to implement and test the rule correctly.', 'best-practices.md#proof-management'],
    check: ['check: validate once, carry the result', 'A check runs validation. Its successful branch returns a value with evidence attached; its failure branch rejects the input. In the invoice example, comparing the customer fields establishes ForCustomer for those particular values. Downstream functions can require that evidence instead of relying on every caller to remember validation.', 'best-practices.md#proof-management'],
    ':::': ['::: means “with evidence of this rule”', 'The value on the left carries the fact on the right. Invoice ::: ForCustomer invoice customer is still an invoice, with evidence about that invoice and customer. Passing an unchecked value does not satisfy the signature. It does not authenticate a customer: a service must obtain that identity from trusted authentication.', 'best-practices.md#proof-management'],
    ...
    record: ['record: related values, with named fields', 'A record describes the fields a value must contain. Invoice has an id and a customer. A record type alone does not prove that a particular user may access it; that separate rule is what the customer check expresses.', 'MANUAL.md'],
    ...
    test: ['test: exercise a concrete case', 'A test documents an expected outcome and runs it against your code. The invoice example tests both a matching customer and a rejected mismatch. The browser checks test code but does not execute it; run the tests locally. Compiler checks and runtime examples answer different questions.', 'MANUAL.md'],
