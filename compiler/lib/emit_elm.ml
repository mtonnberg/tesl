...
let emit_elm ?module_name_override (m : module_form) : string =
  ...
  (* Human actions: per-server `humanActions` handoff types (see the section
     emitted below).  A server contributes a `<Server>HumanAction` tag union, a
     `<Server>HumanActionRequest` record, and its decoder. *)
  let cap_first s =
    if s = "" then s
    else String.make 1 (Char.uppercase_ascii s.[0]) ^ String.sub s 1 (String.length s - 1) in
  let lower_first s =
    if s = "" then s
    else String.make 1 (Char.lowercase_ascii s.[0]) ^ String.sub s 1 (String.length s - 1) in
  let human_action_union_name srv = srv ^ "HumanAction" in
  let human_action_request_name srv = srv ^ "HumanActionRequest" in
  let human_action_decoder_name srv = lower_first srv ^ "HumanActionRequestDecoder" in
  let servers = List.filter_map (function
    | DServer (sv : server_form) -> Some sv
    | _ -> None
  ) m.decls in
  let human_action_exports = List.concat_map (fun (sv : server_form) ->
    [ human_action_union_name sv.name ^ "(..)";
      human_action_request_name sv.name;
      human_action_decoder_name sv.name ]
  ) servers in
...
" source_base;
  ...
  (* For each server, a typed decoder for the `human-action-request` descriptor
     the agent loop emits when it calls a `humanActions` tool it may NOT perform
     itself.  The tag union has one constructor per server endpoint (the tool
     name the runtime puts in the descriptor's `action`) — a COMPILE-TIME
     allowlist: the decoder REJECTS any `action` the agent did not have, and the
     real endpoint URL is resolved by the generated endpoint stub the app calls
     in its `case` arm (never taken from the wire), so the agent can neither
     fabricate an action, relabel it, nor redirect it.  `args` is carried as raw
     JSON (the model's advisory prefill); pass it to the matching generated
     endpoint function, which decodes + validates it and the server re-checks
     auth.  `handle` correlates the completed action back to the conversation
     for a resume-after turn. *)
  if servers <> [] then begin
    add "-- ---------------------------------------------------------------------------\n";
    add "-- Human actions (agent \xE2\x86\x92 human handoff)\n";
    add "-- ---------------------------------------------------------------------------\n\n";
    List.iter (fun (sv : server_form) ->
      let uname = human_action_union_name sv.name in
      ...
      addf "%s =\n" dname;
      addf "    D.map2 %s\n" rname;
      add "        (D.field \"handle\" D.string)\n";
      add "        (D.field \"action\" D.string\n";
      add "            |> D.andThen\n";
      add "                (\\tag ->\n";
      add "                    case tag of\n";
      List.iter (fun tool ->
        addf "                        %S ->\n" tool;
        addf "                            D.map %s (D.field \"args\" D.value)\n\n" (ctor tool)
      ) tools;
      add "                        _ ->\n";
      addf "                            D.fail (\"Unknown %s human action: \" ++ tag)\n" sv.name;
      add "                )\n";
      add "        )\n\n"
    ) servers
  end;
