...
let entity ~reverse (e:R.entity) = list ([atom e.source.identity;atom e.source.table.name;
 ...
 (if reverse then [list (List.map (fun (w:R.reverse_write) ->
   list [atom w.previous;atom w.current;atom w.physical.name]) e.reverse_writes)] else []))
let version_node ~family ~namespace plan (version:R.version) =
 ...
 list ([atom (if version.requires_contract_version<>None then "tesl-retained-physical-version-v3" else if reverse then "tesl-retained-physical-version-v2" else "tesl-retained-physical-version-v1");atom family;atom namespace;int version.version;
 ...
 Option.fold ~none:[] ~some:(fun v -> [int v]) version.requires_contract_version)
