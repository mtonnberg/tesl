...
let check_query_binder_shadowing (decls : top_decl list) : validation_error list =
  ...
  let binder_of (e : expr) =
    ...
    | ESqlQuery { query; _ } ->
      (match query with
       | QuerySelect (seed, _) -> Some (seed.binder, "select")
       | QueryUpdate update -> Some (update.binder, "update")
       | QueryDelete (seed, _) -> Some (seed.binder, "delete")
       | QueryInsert _ | QueryInsertMany _ | QueryUpsert _ -> None)
    | _ ->
