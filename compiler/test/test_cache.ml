...
let test_parse_cache_no_default_ttl () =
  let src = module_ (with_db "cache NoTtl = Cache { database: MainDB valueType: String }\n") in
...
let test_parse_cache_int_value_type () =
  let src = module_ (with_db "cache CountCache = Cache { database: MainDB valueType: Int }\n") in
...
let test_struct_cache_no_database () =
  let src = module_ "cache C = Cache { valueType: String }\n" in
  check_err_contains "struct_no_database" src "missing required field `database`"

(** 3.2 Cache with unknown database reference is an error *)
let test_struct_cache_unknown_database () =
  let src = module_ "cache C = Cache { database: UnknownDB valueType: String }\n" in
...
let test_struct_cache_negative_ttl () =
  ...
  let src = module_ (with_db "cache C = Cache { database: MainDB defaultTtl: 0 valueType: String }\n") in
...
let test_struct_cache_valid_ttl () =
  let src = module_ (with_db "cache C = Cache { database: MainDB defaultTtl: 1 valueType: String }\n") in
...
let test_struct_cache_known_database () =
  let src = module_ (with_db "cache C = Cache { database: MainDB valueType: String }\n") in
...
let test_struct_multiple_caches_one_bad_db () =
  let src = module_ (with_db
    "cache GoodCache = Cache { database: MainDB valueType: String }\n\
     cache BadCache = Cache { database: MissingDB valueType: Int }\n") in
...
let test_struct_cache_empty_db_name () =
  let src = module_ "cache C = Cache { database: \"\"  valueType: String }\n" in
  check_err_contains "struct_empty_db" src "must reference a database"

(** 3.9 Cache with large valid TTL passes *)
let test_struct_cache_large_ttl () =
  let src = module_ (with_db "cache C = Cache { database: MainDB defaultTtl: 86400 valueType: String }\n") in
...
let test_struct_cache_does_not_affect_queues () =
  let src = module_ (with_db "queue Q = Queue { database: MainDB  jobs: [] }\n") in
