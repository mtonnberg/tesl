...
handler put missingReturning() -> String requires [dbWrite Note] =
 ...
 row.title
server NotesServer for NotesApi { health, one, all, addOne, addTwo, addThree, addFour, addFive, addReject, editOne, editAll, conflict, effects, empty, returning, missingReturning, ambiguousReturning }
...
let export ?(contracts=false) destination=project (fun root save path ->
...
let nominal_key_boundaries () = project ~before:(key_schema "V1" false) ~after:(key_schema "VCurrent" true) (fun root save path ->
 ...
 refused "MIG018" edge;
 refused "T001" (replace "id: Schema.Notes.VCurrent.Key { value: old.id.value }" "id: old.id" edge);
...
let ()=match Sys.getenv_opt "TESL_ROW_JSON_KEY_EXPORT",Sys.getenv_opt "TESL_ROW_SETTLED_EXPORT",Sys.getenv_opt "TESL_ROW_ACCESS_EXPORT" with
 | Some root,_,_ -> export_keys root
 | None,Some root,_ -> export ~contracts:true root
 | None,None,Some root -> export root
 | None,None,None -> run "ordinary typed migration queries" ["access",[test_case "actual unchanged V1/V2 App handlers on PostgreSQL" `Quick native;
