...
(define-handler
  (echoUser [body : User])
  #:returns User
  (thsl-src! "tests/emit-incidentals-regressions.tesl" 113 (list (cons 'body *body)) (lambda () (tesl-record-update *body (hash 'id (raw-value (raw-value (UserId "echoed"))))))))
...
(define-handler
  (echoAcct [body : Acct])
  #:returns Acct
  (thsl-src! "tests/emit-incidentals-regressions.tesl" 120 (list (cons 'body *body)) (lambda () (tesl-record-update *body (hash 'id (raw-value (raw-value (AcctId "echoed"))))))))
    ...
    (call-with-fresh-memory-db '()
      (lambda ()
        (call-with-api-test-subscriptions
          (lambda ()
            (define r (thsl-src! "tests/emit-incidentals-regressions.tesl" 150 (list) (lambda () (dispatch-api-test-request MainServer 'get (list "users" "u-9") #:headers (hash) #:capabilities '()))))
  ...
  (test-case "newtype field decodes + re-encodes (stringCodec spelling)"
    (call-with-fresh-memory-db '()
      (lambda ()
        (call-with-api-test-subscriptions
          (lambda ()
            (define r (thsl-src! "tests/emit-incidentals-regressions.tesl" 157 (list) (lambda () (dispatch-api-test-request MainServer 'post (list "users" "echo") #:headers (hash) #:body (hash (string->symbol "id") "ignored" (string->symbol "name") "bob") #:capabilities '()))))
  ...
  (test-case "newtype field encodes transparently (with_codec TypeName spelling)"
    (call-with-fresh-memory-db '()
      (lambda ()
        (call-with-api-test-subscriptions
          (lambda ()
            (define r (thsl-src! "tests/emit-incidentals-regressions.tesl" 164 (list) (lambda () (dispatch-api-test-request MainServer 'get (list "accts" "a-1") #:headers (hash) #:capabilities '()))))
  ...
  (test-case "newtype field decodes + re-encodes (with_codec TypeName spelling)"
    (call-with-fresh-memory-db '()
      (lambda ()
        (call-with-api-test-subscriptions
          (lambda ()
            (define r (thsl-src! "tests/emit-incidentals-regressions.tesl" 171 (list) (lambda () (dispatch-api-test-request MainServer 'post (list "accts" "echo") #:headers (hash) #:body (hash (string->symbol "id") "ignored" (string->symbol "name") "eve") #:capabilities '()))))
