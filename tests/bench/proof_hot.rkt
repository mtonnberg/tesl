...
(define/pow
  ...
  #:returns Integer
  (thsl-src! "tests/bench/proof_hot.tesl" 28 (list (cons 'a *a) (cons 'b *b) (cons 'c *c)) (lambda () (+ (+ *a *b) *c))))

(module+ test
  (require rackunit)
  (test-case "hot path sums three bounded values"
    (call-with-fresh-memory-db '() (lambda ()
  (define one (thsl-src! "tests/bench/proof_hot.tesl" 31 (list) (lambda () 1)))
  (define two (thsl-src! "tests/bench/proof_hot.tesl" 32 (list (cons 'one one)) (lambda () 2)))
  (define three (thsl-src! "tests/bench/proof_hot.tesl" 33 (list (cons 'two two) (cons 'one one)) (lambda () 3)))
  ...
  (check-equal? (raw-value (thsl-src! "tests/bench/proof_hot.tesl" 37 (list (cons 'c c) (cons 'b b) (cons 'a a) (cons 'three three) (cons 'two two) (cons 'one one)) (lambda () (hotPath a b c)))) 6)
    ))
  )
