...
(define/pow
  ...
  #:returns String
  (thsl-src! "tests/agent-feature-tests.tesl" 144 (list (cons 'args *args)) (lambda () (raw-value (tesl_import_String_concat "weather in " (tesl-dot/runtime args 'city 'WeatherArgs))))))

(define/pow
  (dispatchAdd [args : AddArgs])
  #:returns String
  (thsl-src! "tests/agent-feature-tests.tesl" 147 (list (cons 'args *args)) (lambda () (raw-value (tesl_import_String_concat "sum computed for " (raw-value (tesl_import_String_concat (raw-value (intToString (tesl-dot/runtime args 'a 'AddArgs))) (raw-value (tesl_import_String_concat " + " (raw-value (intToString (tesl-dot/runtime args 'b 'AddArgs))))))))))))

(define/pow
  (dispatchFlag [args : FlagArgs])
  #:returns String
  (thsl-src! "tests/agent-feature-tests.tesl" 150 (list (cons 'args *args)) (lambda () (if (tesl-dot/runtime args 'on 'FlagArgs) (raw-value "flag is on") (raw-value "flag is off")))))
...
(module+ test
  ...
  (test-case "askFor decodes a typed value on the first reply"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 407 (list (cons 'summary summary) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime summary 'title 'Summary)))) "All good")
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 408 (list (cons 'summary summary) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime summary 'score 'Summary)))) 42)
    )
    ))
  )
  ...
  (test-case "askFor decodes a single-field record"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 414 (list (cons 's s) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime s 'label 'Sentiment)))) "positive")
    )
    ))
  )
  ...
  (test-case "askFor through a wrapper handler decodes the value"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 419 (list (cons 's s)) (lambda () (raw-value (tesl-dot/runtime s 'label 'Sentiment)))) "positive")
    )
    ))
  )
  ...
  (test-case "askFor succeeds with maxRetries of 1 when the first reply is valid"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 425 (list (cons 'summary summary) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime summary 'title 'Summary)))) "One shot")
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 426 (list (cons 'summary summary) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime summary 'score 'Summary)))) 1)
    )
    ))
  )
  ...
  (test-case "askFor decodes a negative integer field"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 432 (list (cons 'summary summary) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime summary 'score 'Summary)))) -5)
    )
    ))
  )
  ...
  (test-case "askFor retries after one decode failure then succeeds"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 442 (list (cons 'summary summary) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime summary 'title 'Summary)))) "Recovered")
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 443 (list (cons 'summary summary) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime summary 'score 'Summary)))) 7)
    )
    ))
  )
  ...
  (test-case "askFor retries twice (two bad replies) then succeeds on the third"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 449 (list (cons 'summary summary) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime summary 'title 'Summary)))) "Third")
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 450 (list (cons 'summary summary) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime summary 'score 'Summary)))) 3)
    )
    ))
  )
  ...
  (test-case "askFor retry recovers a single-field decoder"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 456 (list (cons 's s) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime s 'label 'Sentiment)))) "neutral")
    )
    ))
  )
  ...
  (test-case "askFor retry consumes the bad reply before the good one"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 464 (list (cons 'a2 a2) (cons 'a1 a1)) (lambda () (raw-value (tesl-dot/runtime (raw-value (askFor (raw-value a1) "x" decodeSummary 2)) 'title 'Summary)))) "A")
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 465 (list (cons 'a2 a2) (cons 'a1 a1)) (lambda () (raw-value (tesl-dot/runtime (raw-value (askFor (raw-value a2) "y" decodeSummary 2)) 'title 'Summary)))) "B")
    )
    ))
  )
  ...
  (test-case "capability flows: askFor under supportBot"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 651 (list (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime (raw-value (askFor (raw-value agent) "hi" decodeSentiment 2)) 'label 'Sentiment)))) "ok")
    )
    ))
  )
  ...
  (test-case "decodeAs standalone decodes a JSON string to a typed value"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 666 (list (cons 's s)) (lambda () (raw-value (tesl-dot/runtime s 'title 'Summary)))) "direct")
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 667 (list (cons 's s)) (lambda () (raw-value (tesl-dot/runtime s 'score 'Summary)))) 9)
    )
    ))
  )
  ...
  (test-case "tool loop then a separate structured-output ask compose"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 676 (list (cons 's s) (cons 'reply reply) (cons 'agent agent) (cons 'weatherTool weatherTool)) (lambda () (raw-value (tesl-dot/runtime s 'label 'Sentiment)))) "positive")
    )
    ))
  )
  ...
  (test-case "converse reply feeds a downstream structured decode of the same shape"
    ...
    (check-equal? (thsl-src! "tests/agent-feature-tests.tesl" 684 (list (cons 's s) (cons 'txt txt) (cons 'turn turn) (cons 'agent agent)) (lambda () (raw-value (tesl-dot/runtime s 'label 'Sentiment)))) "happy")
    )
    ))
  )
