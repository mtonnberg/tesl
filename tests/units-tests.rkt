...
(define/pow
  (pace [d : Real] [t : Real])
  #:returns Real
  (thsl-src! "tests/units-tests.tesl" 82 (list (cons 'd *d) (cons 't *t)) (lambda () (let/check ([tesl-checked-0 (tesl_import_Units_requireNonZero t)]) (let ([tc tesl-checked-0]) (/ *d (raw-value tc)))))))

(define/pow
  (lengthRatio [a : Real] [b : Real])
  #:returns Real
  (thsl-src! "tests/units-tests.tesl" 87 (list (cons 'a *a) (cons 'b *b)) (lambda () (let/check ([tesl-checked-1 (tesl_import_Units_requireNonZero b)]) (let ([bc tesl-checked-1]) (/ *a (raw-value bc)))))))

(define/pow
  (frequencyOf [period : Real])
  #:returns Real
  (thsl-src! "tests/units-tests.tesl" 92 (list (cons 'period *period)) (lambda () (let/check ([tesl-checked-2 (tesl_import_Units_requireNonZero period)]) (let ([pc tesl-checked-2]) (/ 1. (raw-value pc)))))))
...
(module+ test
  ...
  (test-case "same-dimension comparison is numeric"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/units-tests.tesl" 147 (list) (lambda () (tesl-gt? (raw-value (tesl_import_Length_meters 2.)) (raw-value (tesl_import_Length_meters 1.)))))) #t)
