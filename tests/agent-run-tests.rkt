        ...
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (runRead runWrite runPubSub runQueue runWorker)
              (define stream (thsl-src! "tests/agent-run-tests.tesl" 190 (list) (lambda () (subscribe AgentRunServer-sse-routes (list "events" "runs" "run-1") #:headers (hash) #:name "/events/runs/run-1"))))
              (define posted (thsl-src! "tests/agent-run-tests.tesl" 192 (list (cons 'stream stream)) (lambda () (dispatch-api-test-request AgentRunServer 'post (list "runs") #:headers (hash) #:body (hash (string->symbol "prompt") "please echo") #:capabilities (list runRead runWrite runPubSub runQueue runWorker)))))
              ...
              (check-true (raw-value (thsl-src! "tests/agent-run-tests.tesl" 214 (list (cons 'second second) (cons 'first first) (cons 'events events) (cons 'job job) (cons 'done done) (cons 'posted posted) (cons 'stream stream)) (lambda () (includesWhere (hash 'tag "Step" 'fields (hash 'content "tool: echo")) (raw-value events))))))
              (check-true (raw-value (thsl-src! "tests/agent-run-tests.tesl" 215 (list (cons 'second second) (cons 'first first) (cons 'events events) (cons 'job job) (cons 'done done) (cons 'posted posted) (cons 'stream stream)) (lambda () (includesWhere (hash 'tag "Step" 'fields (hash 'content "text: All done.")) (raw-value events))))))
            )
          ))
      ))
  )
