...
(module+ test
  ...
  (test-case "shared-field read in where value operand does not trap (#27)"
    ...
    (define tesl-ignored-0 (thsl-src! "tests/sql-where-hint-tests.tesl" 33 (list) (lambda () (insert-one! OrgW (hash 'id "o1" 'name "acme")))))
    (define pr (thsl-src! "tests/sql-where-hint-tests.tesl" 34 (list) (lambda () (hash 'id "p1" 'name "acme"))))
  ...
  (test-case "compound where with two shared-field value operands (#27)"
    (call-with-fresh-memory-db '() (lambda ()
    (with-capabilities (dbRead dbWrite)
    (define tesl-ignored-2 (thsl-src! "tests/sql-where-hint-tests.tesl" 41 (list) (lambda () (insert-one! OrgW (hash 'id "same" 'name "beta")))))
    (define pr (thsl-src! "tests/sql-where-hint-tests.tesl" 42 (list) (lambda () (hash 'id "same" 'name "beta"))))
  ...
  (test-case "no match returns Nothing without trapping (#27)"
    (call-with-fresh-memory-db '() (lambda ()
    (with-capabilities (dbRead dbWrite)
    (define pr (thsl-src! "tests/sql-where-hint-tests.tesl" 47 (list) (lambda () (hash 'id "px" 'name "no-such-org"))))
