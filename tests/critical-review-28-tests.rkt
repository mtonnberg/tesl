...
(require
  ...
  (only-in tesl/tesl/float Float [Float.requireNonZero tesl_import_Float_requireNonZero] [Float.requireNonNegative tesl_import_Float_requireNonNegative] FloatNonZero [Float.div tesl_import_Float_div] [Float.sqrt tesl_import_Float_sqrt])
...
(define-checker
  ...
  (thsl-src! "tests/critical-review-28-tests.tesl" 137 (list (cons 'n *n)) (lambda () (if (and (tesl-ge? *n 0) (tesl-le? *n 1000)) (accept (IsRange28 n) #:value *n) (reject "out of 0\u20131000 range" #:http-code 400)))))
...
(define-checker
  ...
  (thsl-src! "tests/critical-review-28-tests.tesl" 178 (list (cons 's *s)) (lambda () (if (tesl-le? (raw-value (tesl_import_String_length *s)) 50) (accept (IsShort28 s) #:value *s) (reject "too long" #:http-code 400)))))
...
(define-checker
  ...
  (thsl-src! "tests/critical-review-28-tests.tesl" 275 (list (cons 'email *email)) (lambda () (if (and (raw-value (tesl_import_String_contains *email "@")) (tesl-ge? (raw-value (tesl_import_String_length *email)) 3)) (accept (IsValidEmail28 email) #:value *email) (reject "invalid email" #:http-code 400)))))
...
(define-checker
  ...
  (thsl-src! "tests/critical-review-28-tests.tesl" 474 (list (cons 'lo *lo) (cons 'hi *hi) (cons 'n *n)) (lambda () (if (and (tesl-ge? *n *lo) (tesl-le? *n *hi)) (accept (Bounded28 lo hi n) #:value *n) (reject "value out of bounds" #:http-code 400)))))
...
(define-trusted
  ...
  (thsl-src! "tests/critical-review-28-tests.tesl" 587 (list (cons 'n *n)) (lambda () (if (tesl-lt? *n 10) (Something (trusted-proof (IsSmall28 n))) Nothing))))
...
(define-checker
  ...
  (thsl-src! "tests/critical-review-28-tests.tesl" 662 (list (cons 's *s)) (lambda () (if (tesl-ge? (raw-value (tesl_import_String_length *s)) 5) (accept (IsLong28 s) #:value *s) (reject "too short" #:http-code 400)))))
...
(define/pow
  ...
  #:returns (Either String Integer)
  (thsl-src! "tests/critical-review-28-tests.tesl" 931 (list (cons 'a *a) (cons 'b *b)) (lambda () (if (tesl-equal? *b 0) (raw-value (raw-value (Left "zero"))) (let/check ([tesl-checked-24 (tesl_import_Int_nonZero b)]) (let ([nb tesl-checked-24]) (raw-value (raw-value (Right (tesl_import_Int_divide *a nb))))))))))

(define/pow
  (doubleRight28 [e : (Either String Integer)])
  #:returns (Either String Integer)
  (thsl-src! "tests/critical-review-28-tests.tesl" 938 (list (cons 'e *e)) (lambda () (raw-value (tesl_import_Either_map (raw-value (lambda (tesl-p-25-0) (multiply28 2 tesl-p-25-0))) *e)))))

(define/pow
  (callCheck28 [raw : Integer])
  #:returns Integer
  (thsl-src! "tests/critical-review-28-tests.tesl" 972 (list (cons 'raw *raw)) (lambda () (let/check ([tesl-checked-26 (checkRange28 raw)]) (let ([v tesl-checked-26]) (raw-value (requiresRange28 v)))))))

(define/pow
  (listLengthNonNeg28 [xs : (List Integer)])
  #:returns Boolean
  (thsl-src! "tests/critical-review-28-tests.tesl" 1108 (list (cons 'xs *xs)) (lambda () (tesl-ge? (raw-value (tesl_import_List_length *xs)) 0))))

(define/pow
  (listSumReverseSame28 [xs : (List Integer)])
  #:returns Boolean
  (thsl-src! "tests/critical-review-28-tests.tesl" 1111 (list (cons 'xs *xs)) (lambda () (tesl-equal? (raw-value (tesl_import_List_sum *xs)) (raw-value (tesl_import_List_sum (raw-value (tesl_import_List_reverse *xs))))))))
...
(module+ test
  ...
  (test-case "T17g: max safe Int is within range"
    (call-with-fresh-memory-db '() (lambda ()
  (check-true (thsl-src! "tests/critical-review-28-tests.tesl" 801 (list) (lambda () (tesl-gt? (raw-value (maxSafeInt28)) 1000000000))))
    ))
  )
  ...
  (test-case "T21c: float sqrt"
    (call-with-fresh-memory-db '() (lambda ()
  (define nineRaw (thsl-src! "tests/critical-review-28-tests.tesl" 911 (list) (lambda () 9.)))
  (define tesl-checked-32 (tesl_import_Float_requireNonNegative nineRaw))
  (when (check-fail? tesl-checked-32)
    (raise-user-error 'tesl-test "unexpected failure in let nine: ~a" (check-fail-message tesl-checked-32)))
  (define nine tesl-checked-32)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 913 (list (cons 'nine nine) (cons 'nineRaw nineRaw)) (lambda () (raw-value (tesl_import_Float_sqrt nine))))) 3.)
  (define fourRaw (thsl-src! "tests/critical-review-28-tests.tesl" 914 (list (cons 'nine nine) (cons 'nineRaw nineRaw)) (lambda () 4.)))
  (define tesl-checked-33 (tesl_import_Float_requireNonNegative fourRaw))
  (when (check-fail? tesl-checked-33)
    (raise-user-error 'tesl-test "unexpected failure in let four: ~a" (check-fail-message tesl-checked-33)))
  (define four tesl-checked-33)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 916 (list (cons 'four four) (cons 'fourRaw fourRaw) (cons 'nine nine) (cons 'nineRaw nineRaw)) (lambda () (raw-value (tesl_import_Float_sqrt four))))) 2.)
  (define zeroRaw (thsl-src! "tests/critical-review-28-tests.tesl" 917 (list (cons 'four four) (cons 'fourRaw fourRaw) (cons 'nine nine) (cons 'nineRaw nineRaw)) (lambda () 0.)))
  (define tesl-checked-34 (tesl_import_Float_requireNonNegative zeroRaw))
  (when (check-fail? tesl-checked-34)
    (raise-user-error 'tesl-test "unexpected failure in let zero: ~a" (check-fail-message tesl-checked-34)))
  (define zero tesl-checked-34)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 919 (list (cons 'zero zero) (cons 'zeroRaw zeroRaw) (cons 'four four) (cons 'fourRaw fourRaw) (cons 'nine nine) (cons 'nineRaw nineRaw)) (lambda () (raw-value (tesl_import_Float_sqrt zero))))) 0.)
    ))
  )

  (test-case "T21d: float arithmetic"
    (call-with-fresh-memory-db '() (lambda ()
  (define result (thsl-src! "tests/critical-review-28-tests.tesl" 923 (list) (lambda () (safeFloatDiv28 1. 2.))))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 924 (list (cons 'result result)) (lambda () result))) (raw-value (Right 0.5)))
    ))
  )

  (test-case "T22a: Either.map over Right"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 941 (list) (lambda () (doubleRight28 (Right 5))))) (raw-value (Right 10)))
    ))
  )

  (test-case "T22b: Either.map over Left is identity"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 945 (list) (lambda () (doubleRight28 (Left "error"))))) (raw-value (Left "error")))
    ))
  )

  (test-case "T22c: Either.withDefault on Left"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 949 (list) (lambda () (raw-value (tesl_import_Either_withDefault 99 (Left "err")))))) 99)
    ))
  )

  (test-case "T22d: Either.withDefault on Right"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 953 (list) (lambda () (raw-value (tesl_import_Either_withDefault 99 (Right 42)))))) 42)
    ))
  )

  (test-case "T22e: Either.andThen chains operations"
    (call-with-fresh-memory-db '() (lambda ()
  (define result (thsl-src! "tests/critical-review-28-tests.tesl" 959 (list) (lambda () (raw-value (tesl_import_Either_andThen (raw-value (lambda (tesl-p-35-0) (divOrError28 100 tesl-p-35-0))) (raw-value (divOrError28 10 2)))))))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 960 (list (cons 'result result)) (lambda () result))) (raw-value (Right 20)))
    ))
  )

  (test-case "T22f: Either.andThen short-circuits on Left"
    (call-with-fresh-memory-db '() (lambda ()
  (define result (thsl-src! "tests/critical-review-28-tests.tesl" 964 (list) (lambda () (raw-value (tesl_import_Either_andThen (raw-value (lambda (tesl-p-36-0) (divOrError28 100 tesl-p-36-0))) (raw-value (divOrError28 10 0)))))))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 965 (list (cons 'result result)) (lambda () result))) (raw-value (Left "zero")))
    ))
  )

  (test-case "T23a: fn can call check and use result with proof"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 976 (list) (lambda () (callCheck28 500)))) 501)
    ))
  )

  (test-case "T23b: fn calling check fails propagates failure"
    (call-with-fresh-memory-db '() (lambda ()
  (let ([tesl-ef-result (with-handlers ([exn:fail? (lambda (e) 'tesl-exception)]) (thsl-src! "tests/critical-review-28-tests.tesl" 980 (list) (lambda ()
                          (callCheck28 -1))))])
    (check-true (or (eq? tesl-ef-result 'tesl-exception) (check-fail? tesl-ef-result))
                "expected failure: callCheck28 -1"))
  (let ([tesl-ef-result (with-handlers ([exn:fail? (lambda (e) 'tesl-exception)]) (thsl-src! "tests/critical-review-28-tests.tesl" 981 (list) (lambda ()
                          (callCheck28 1001))))])
    (check-true (or (eq? tesl-ef-result 'tesl-exception) (check-fail? tesl-ef-result))
  ...
  (test-case "T24a: String.toUpper"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 988 (list) (lambda () (tesl_import_String_toUpper "hello")))) "HELLO")
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 989 (list) (lambda () (tesl_import_String_toUpper "")))) "")
    ))
  )

  (test-case "T24b: String.toLower"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 993 (list) (lambda () (tesl_import_String_toLower "HELLO")))) "hello")
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 994 (list) (lambda () (tesl_import_String_toLower "MiXeD")))) "mixed")
    ))
  )

  (test-case "T24c: String.trim"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 998 (list) (lambda () (tesl_import_String_trim "  hello  ")))) "hello")
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 999 (list) (lambda () (tesl_import_String_trim "hello")))) "hello")
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1000 (list) (lambda () (tesl_import_String_trim "  ")))) "")
    ))
  )

  (test-case "T24d: String.startsWith"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1004 (list) (lambda () (tesl_import_String_startsWith "hello world" "hello")))) #t)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1005 (list) (lambda () (tesl_import_String_startsWith "hello world" "world")))) #f)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1006 (list) (lambda () (tesl_import_String_startsWith "" "")))) #t)
    ))
  )

  (test-case "T24e: String.endsWith"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1010 (list) (lambda () (tesl_import_String_endsWith "hello world" "world")))) #t)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1011 (list) (lambda () (tesl_import_String_endsWith "hello world" "hello")))) #f)
    ))
  )

  (test-case "T24f: String.split and join round-trip"
    (call-with-fresh-memory-db '() (lambda ()
  (define parts (thsl-src! "tests/critical-review-28-tests.tesl" 1015 (list) (lambda () (tesl_import_String_split "a,b,c" ","))))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1016 (list (cons 'parts parts)) (lambda () (tesl_import_String_join (raw-value parts) ",")))) "a,b,c")
    ))
  )

  (test-case "T24g: String.replace"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1020 (list) (lambda () (tesl_import_String_replace "hello world" "world" "tesl")))) "hello tesl")
    ))
  )

  (test-case "T24h: String.length"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1024 (list) (lambda () (tesl_import_String_length "hello")))) 5)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1025 (list) (lambda () (tesl_import_String_length "")))) 0)
    ))
  )

  (test-case "T25a: List.contains"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1032 (list) (lambda () (raw-value (tesl_import_List_contains 2 (list 1 2 3)))))) #t)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1033 (list) (lambda () (raw-value (tesl_import_List_contains 4 (list 1 2 3)))))) #f)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1034 (list) (lambda () (raw-value (tesl_import_List_contains 1 (list)))))) #f)
    ))
  )

  (test-case "T25b: List.find"
    (call-with-fresh-memory-db '() (lambda ()
  (define result (thsl-src! "tests/critical-review-28-tests.tesl" 1038 (list) (lambda () (raw-value (tesl_import_List_find (let () (define/pow (tesl-lambda-37 [n : Integer]) #:returns Boolean (tesl-gt? *n 2)) tesl-lambda-37) (list 1 2 3 4))))))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1039 (list (cons 'result result)) (lambda () result))) (raw-value (Something 3)))
  (define notFound (thsl-src! "tests/critical-review-28-tests.tesl" 1040 (list (cons 'result result)) (lambda () (raw-value (tesl_import_List_find (let () (define/pow (tesl-lambda-38 [n : Integer]) #:returns Boolean (tesl-gt? *n 10)) tesl-lambda-38) (list 1 2))))))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1041 (list (cons 'notFound notFound) (cons 'result result)) (lambda () notFound))) Nothing)
    ))
  )

  (test-case "T25c: List.take with proof"
    (call-with-fresh-memory-db '() (lambda ()
  (define n3 (thsl-src! "tests/critical-review-28-tests.tesl" 1045 (list) (lambda () 3)))
  (define tesl-checked-39 (tesl_import_Int_nonNegative n3))
  (when (check-fail? tesl-checked-39)
    (raise-user-error 'tesl-test "unexpected failure in let count: ~a" (check-fail-message tesl-checked-39)))
  (define count tesl-checked-39)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1047 (list (cons 'count count) (cons 'n3 n3)) (lambda () (tesl_import_List_take count (list 1 2 3 4 5))))) (list 1 2 3))
  (define n0 (thsl-src! "tests/critical-review-28-tests.tesl" 1048 (list (cons 'count count) (cons 'n3 n3)) (lambda () 0)))
  (define tesl-checked-40 (tesl_import_Int_nonNegative n0))
  (when (check-fail? tesl-checked-40)
    (raise-user-error 'tesl-test "unexpected failure in let zero: ~a" (check-fail-message tesl-checked-40)))
  (define zero tesl-checked-40)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1050 (list (cons 'zero zero) (cons 'n0 n0) (cons 'count count) (cons 'n3 n3)) (lambda () (tesl_import_List_take zero (list 1 2 3))))) (list))
    ))
  )

  (test-case "T25d: List.drop with proof"
    (call-with-fresh-memory-db '() (lambda ()
  (define n2 (thsl-src! "tests/critical-review-28-tests.tesl" 1054 (list) (lambda () 2)))
  (define tesl-checked-41 (tesl_import_Int_nonNegative n2))
  (when (check-fail? tesl-checked-41)
    (raise-user-error 'tesl-test "unexpected failure in let count: ~a" (check-fail-message tesl-checked-41)))
  (define count tesl-checked-41)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1056 (list (cons 'count count) (cons 'n2 n2)) (lambda () (tesl_import_List_drop count (list 1 2 3 4 5))))) (list 3 4 5))
    ))
  )

  (test-case "T25e: List.sum"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1060 (list) (lambda () (raw-value (tesl_import_List_sum (list 1 2 3 4 5)))))) 15)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1061 (list) (lambda () (raw-value (tesl_import_List_sum (list)))))) 0)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1062 (list) (lambda () (raw-value (tesl_import_List_sum (list 0 0 0)))))) 0)
    ))
  )

  (test-case "T25f: List.product"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1066 (list) (lambda () (raw-value (tesl_import_List_product (list 1 2 3 4)))))) 24)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1067 (list) (lambda () (raw-value (tesl_import_List_product (list)))))) 1)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1068 (list) (lambda () (raw-value (tesl_import_List_product (list 0 1 2)))))) 0)
    ))
  )

  (test-case "T25g: List.reverse"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1072 (list) (lambda () (tesl_import_List_reverse (list 1 2 3))))) (list 3 2 1))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1073 (list) (lambda () (tesl_import_List_reverse (list))))) (list))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1074 (list) (lambda () (tesl_import_List_reverse (list 1))))) (list 1))
    ))
  )

  (test-case "T25h: List.unique"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1078 (list) (lambda () (tesl_import_List_unique (list 1 2 1 3 2))))) (list 1 2 3))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1079 (list) (lambda () (tesl_import_List_unique (list))))) (list))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1080 (list) (lambda () (tesl_import_List_unique (list 1 1 1))))) (list 1))
    ))
  )

  (test-case "T25i: filterCheck then map"
    (call-with-fresh-memory-db '() (lambda ()
  (define evens (thsl-src! "tests/critical-review-28-tests.tesl" 1084 (list) (lambda () (tesl_import_List_filterCheck checkEven28 (list 1 2 3 4 5 6)))))
  (define doubled (thsl-src! "tests/critical-review-28-tests.tesl" 1085 (list (cons 'evens evens)) (lambda () (tesl_import_List_map (let () (define/pow (tesl-lambda-42 [n : Integer]) #:returns Integer (let ([n (tesl-establish-param-proof n *n `(IsEven28 ,n))]) (* *n 2))) tesl-lambda-42) (raw-value evens)))))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1086 (list (cons 'doubled doubled) (cons 'evens evens)) (lambda () doubled))) (list 4 8 12))
    ))
  )

  (test-case "T25j: List.any and List.all"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1090 (list) (lambda () (raw-value (tesl_import_List_any (let () (define/pow (tesl-lambda-43 [n : Integer]) #:returns Boolean (tesl-gt? *n 3)) tesl-lambda-43) (list 1 2 3 4)))))) #t)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1091 (list) (lambda () (raw-value (tesl_import_List_any (let () (define/pow (tesl-lambda-44 [n : Integer]) #:returns Boolean (tesl-gt? *n 10)) tesl-lambda-44) (list 1 2 3)))))) #f)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1092 (list) (lambda () (raw-value (tesl_import_List_all (let () (define/pow (tesl-lambda-45 [n : Integer]) #:returns Boolean (tesl-gt? *n 0)) tesl-lambda-45) (list 1 2 3)))))) #t)
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1093 (list) (lambda () (raw-value (tesl_import_List_all (let () (define/pow (tesl-lambda-46 [n : Integer]) #:returns Boolean (tesl-gt? *n 1)) tesl-lambda-46) (list 1 2 3)))))) #f)
    ))
  )

  (test-case "T25k: List idempotent sort examples"
    (call-with-fresh-memory-db '() (lambda ()
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1097 (list) (lambda () (tesl_import_List_sort (list 9 1 8 2 7 3))))) (list 1 2 3 7 8 9))
  (check-equal? (raw-value (thsl-src! "tests/critical-review-28-tests.tesl" 1098 (list) (lambda () (raw-value (tesl_import_List_length (raw-value (tesl_import_List_sort (list 3 1 4 1 5 9 2 6)))))))) 8)
    ))
  )
