...
(module+ test
  ...
  (test-case "PosixMillis range in select-where works (was a runtime trap, #28)"
    ...
    (define tesl-ignored-0 (thsl-src! "tests/sql-newtype-range-tests.tesl" 23 (list (cons 'now now)) (lambda () (insert-one! Ev (tesl-hash 'id "a" 'at now)))))
  ...
  (test-case "PosixMillis range excludes out-of-window rows (#28)"
    ...
    (define tesl-ignored-1 (thsl-src! "tests/sql-newtype-range-tests.tesl" 31 (list (cons 'now now)) (lambda () (insert-one! Ev (tesl-hash 'id "b" 'at now)))))
