...
(define-checker
  ...
  (thsl-src! "tests/exists-consume-tests.tesl" 22 (list (cons 's *s)) (lambda () (if (> (raw-value (tesl_import_String_length *s)) 3) (accept (IsTokenId s) #:value *s) (reject "bad token" #:http-code 400)))))
...
(module+ test
  ...
  (test-case "exists result consumed as its underlying String type"
    ...
    (check-true (thsl-src! "tests/exists-consume-tests.tesl" 38 (list (cons 'tok tok)) (lambda () (> (raw-value (tesl_import_String_length (raw-value tok))) 3))))
