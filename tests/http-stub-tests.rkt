...
(define (tesl-codec-decode-SyncRequest-0 _j)
  ...
  (record-value 'SyncRequest (hash 'tag _f_tag)))
        ...
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (queueRead queueWrite webClient)
              ...
              (define queued (thsl-src! "tests/http-stub-tests.tesl" 268 (list) (lambda () (dispatch-api-test-request SyncServer 'post (list "sync") #:headers (hash) #:body (hash (string->symbol "tag") "nightly") #:capabilities (list queueRead queueWrite webClient)))))
        ...
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (queueRead queueWrite webClient)
              ...
              (define queued (thsl-src! "tests/http-stub-tests.tesl" 291 (list) (lambda () (dispatch-api-test-request SyncServer 'post (list "sync") #:headers (hash) #:body (hash (string->symbol "tag") "nightly") #:capabilities (list queueRead queueWrite webClient)))))
