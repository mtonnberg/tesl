...
(define-checker
  ...
  (thsl-src! "tests/secret-proof-composition-tests.tesl" 46 (list (cons 'text *text)) (lambda () (if (tesl-ge? (raw-value (tesl_import_String_length *text)) 8) (accept (LongEnough text) #:value *text) (reject "Password too short" #:http-code 400)))))
    ...
    (call-with-fresh-memory-db '()
      (lambda ()
        (call-with-api-test-subscriptions
          (lambda ()
            (define r (thsl-src! "tests/secret-proof-composition-tests.tesl" 85 (list) (lambda () (dispatch-api-test-request RegisterServer 'post (list "register") #:headers (tesl-hash) #:body (tesl-hash (string->symbol "email") "a@example.com" (string->symbol "password") "short") #:capabilities '()))))
            (check-equal? (raw-value (thsl-src! "tests/secret-proof-composition-tests.tesl" 86 (list (cons 'r r)) (lambda () (api-test-field-access-ref r 'status)))) 400)
          ))
      ))
  )
)
...
(module+ test
  (require rackunit)
  (test-case "a long-enough password validates, then is wrapped as a secret"
    (call-with-fresh-memory-db '()
      (lambda ()
        (call-with-api-test-subscriptions
          (lambda ()
            (define r (thsl-src! "tests/secret-proof-composition-tests.tesl" 90 (list) (lambda () (dispatch-api-test-request RegisterServer 'post (list "register") #:headers (tesl-hash) #:body (tesl-hash (string->symbol "email") "a@example.com" (string->symbol "password") "hunter2222") #:capabilities '()))))
            (check-true (raw-value (thsl-src! "tests/secret-proof-composition-tests.tesl" 91 (list (cons 'r r)) (lambda () (statusOk (raw-value (api-test-field-access-ref r 'status)))))))
          ))
      ))
  )
)
