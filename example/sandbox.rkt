...
(provide shouldWork dummy_add ValidPort IsPositive doSomething doSomething2 ARecord ARecord2 FiveCases CaseOne CaseTwo CaseThree CaseFour CaseFive doSomething3 doSomething2Of3 S3Record S3Record2 shouldWork-signature dummy_add-signature)
...
(define/pow
  ...
  #:returns Integer
  (thsl-src! "example/sandbox.tesl" 36 (list (cons 'x *x)) (lambda () (tesl-dot/runtime x 'foo3 'Sandbox3.S3Record2))))

(define/pow
  (shouldWork8 [x : Sandbox2.ARecord2])
  #:returns Integer
  (thsl-src! "example/sandbox.tesl" 38 (list (cons 'x *x)) (lambda () (tesl-dot/runtime x 'foo2 'Sandbox2.ARecord2))))
...
(define-trusted
  ...
  (thsl-src! "example/sandbox.tesl" 49 (list (cons 'port *port)) (lambda () (if (and (tesl-le? 1 *port) (tesl-le? *port 65535)) (Something (trusted-proof (ValidPort port))) Nothing))))

(define-checker
  (checkPositive [n : Integer])
  #:returns [n : Integer ::: (IsPositive n)]
  (thsl-src! "example/sandbox.tesl" 57 (list (cons 'n *n)) (lambda () (if (tesl-gt? *n 0) (accept (IsPositive n) #:value *n) (reject "Value must be over 0" #:http-code 400)))))
