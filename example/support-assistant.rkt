...
(define SupportAgent
  (__tart_withTools (__tart_defineAgent (raw-value (anthropic (raw-value (requireEnv "ANTHROPIC_API_KEY")) "claude-opus-4-8")) (raw-value "You are a concise customer-support assistant. Never invent order ids.") (raw-value 512)) (list (__tart_tool "lookupOrder" "Look up the shipping status of an order by its id." "{\"type\":\"object\",\"properties\":{\"orderId\":{\"type\":\"string\"}},\"required\":[\"orderId\"]}" (lambda (_args) (__tart_tesl-agent-decode-args _args (list (cons "orderId" 'string)))) (lambda (_decoded) (apply lookupOrder _decoded))) (__tart_tool "refundOrder" "Issue a refund for an order. The model must pass confirmed: true; the guard lives inside the typed value, so \"did the mutation actually happen?\" is answerable by asserting on the tool result, deterministically." "{\"type\":\"object\",\"properties\":{\"orderId\":{\"type\":\"string\"},\"confirmed\":{\"type\":\"boolean\"}},\"required\":[\"orderId\",\"confirmed\"]}" (lambda (_args) (__tart_tesl-agent-decode-args _args (list (cons "orderId" 'string) (cons "confirmed" 'bool)))) (lambda (_decoded) (apply refundOrder _decoded))))))
