...
(define-handler
  ...
  (thsl-src! "example/kanel/KanelBilling.tesl" 73 (list (cons 'session *session) (cons 'orgId *orgId) (cons 'req *req)) (lambda () (let/check ([tesl-checked-2 (checkOrgAdmin (raw-value session.userId) orgId)]) (let ([_adminUserId tesl-checked-2]) (let ([entryCount (select-count (from TimeEntry) (where (==. (entity-field-ref TimeEntry 'orgId) orgId)) (where (==. (entity-field-ref TimeEntry 'invoiceId) "")))]) (if (tesl-equal? *entryCount 0) (reject "no unbilled time entries found for this organization" #:http-code 422) (let ([totalMinutes (select-sum (entity-field-ref TimeEntry 'minutes) (from TimeEntry) (where (==. (entity-field-ref TimeEntry 'orgId) orgId)) (where (==. (entity-field-ref TimeEntry 'invoiceId) "")))]) (let ([invoiceId (generatePrefixedId "inv")]) (call-with-queue-transaction (lambda () (let ([_ (void (update-many! (from TimeEntry) (hash (entity-field-ref TimeEntry 'invoiceId) invoiceId) (where (==. (entity-field-ref TimeEntry 'orgId) orgId)) (where (==. (entity-field-ref TimeEntry 'invoiceId) ""))))]) (pack ([invoiceId]) (insert-one! Invoice (hash 'id invoiceId 'orgId orgId 'status Draft 'totalMinutes *totalMinutes 'notes (raw-value req.notes) 'createdAt (raw-value (nowMillis)) 'approvedAt (raw-value (tesl_import_Time_secondsToPosix 0)))))))))))))))))
