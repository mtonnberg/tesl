...
(define/pow
  ...
  #:capabilities [kanelDbRead]
  #:returns (List Org)
  (let ([found (thsl-src! "example/kanel/KanelOrg.tesl" 246 (list (cons 'acc *acc) (cons 'm *m)) (lambda () (let ([tesl_match (select-one (from Org) (where (==. (entity-field-ref Org 'id) (tesl-dot/runtime m 'orgId))))]) (if tesl_match (Something tesl_match) Nothing))))]) (thsl-src! "example/kanel/KanelOrg.tesl" 247 (list (cons 'found *found) (cons 'acc *acc) (cons 'm *m)) (lambda () (let ([tesl_case_10 (raw-value found)]) (cond [(and (adt-value? *tesl_case_10) (eq? (adt-value-variant *tesl_case_10) 'Nothing)) *acc] [(and (adt-value? *tesl_case_10) (eq? (adt-value-variant *tesl_case_10) 'Something)) (let ([o (hash-ref (adt-value-fields *tesl_case_10) 'value)]) (raw-value (tesl_import_List_append *acc (list *o))))]))))))
