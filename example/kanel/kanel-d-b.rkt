...
(define/pow
  ...
  #:capabilities [kanelDbRead]
  #:returns (List Org)
  (let ([found (let ([tesl_match (select-one (from Org) (where (==. (entity-field-ref Org 'id) (tesl-dot/runtime m 'orgId))))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_0 (raw-value found)]) (cond [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Nothing)) *acc] [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Something)) (let ([o (hash-ref (adt-value-fields *tesl_case_0) 'value)]) (raw-value (tesl_import_List_append *acc (list *o))))]))))

(define/pow
  (dbListOrgs [userId : String])
  #:capabilities [kanelDbRead]
  #:returns (List Org)
  (let ([memberships (select-many (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) userId)))]) (raw-value (tesl_import_List_foldl dbFetchOrgByMembership (list) (raw-value memberships)))))
