...
(define-auther
  ...
  (let ([tesl_case_0 (raw-value (tesl_import_Dict_lookup "kanel_user_id" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Nothing)) (reject "not logged in" #:http-code 401)] [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl_case_0) 'value)]) (let ([found (let ([tesl_match (select-one (from KanelUser) (where (==. (entity-field-ref KanelUser 'id) userId)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_1 (raw-value found)]) (cond [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Nothing)) (reject "session invalid" #:http-code 401)] [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Something)) (let ([user (hash-ref (adt-value-fields *tesl_case_1) 'value)]) (accept Authenticated #:value (KanelSession #:userId (raw-value user.id) #:displayName (raw-value user.displayName) #:email (raw-value user.email))))]))))])))

(define-checker
  (checkOrgMember [userId : String] [orgId : String])
  #:capabilities [kanelDbRead]
  #:returns [userId : String ::: (OrgMember userId orgId)]
  (let ([membership (let ([tesl_match (select-one (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) userId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_2 (raw-value membership)]) (cond [(and (adt-value? *tesl_case_2) (eq? (adt-value-variant *tesl_case_2) 'Nothing)) (reject "not a member of this organization" #:http-code 403)] [(and (adt-value? *tesl_case_2) (eq? (adt-value-variant *tesl_case_2) 'Something)) (accept (OrgMember userId orgId) #:value *userId)]))))
