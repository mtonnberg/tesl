...
(define/pow
  ...
  #:capabilities [notifyWorkerCap kanelPubSub]
  #:returns KanelNotifyJob
  (let ([_ (thsl-src! "example/kanel/KanelBackend.tesl" 203 (list (cons 'job *job)) (lambda () (publish-event! UserNotifications (format "~a" (raw-value job.recipientUserId)) (NotificationFailed (raw-value job.recipientEmail) (raw-value job.subject)))))]) (thsl-src! "example/kanel/KanelBackend.tesl" 204 (list (cons 'job *job)) (lambda () *job))))
