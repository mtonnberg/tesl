...
(define/pow
  ...
  (thsl-src! "example/kanel/KanelBackend.tesl" 193 (list (cons 'job *job)) (lambda () (if (equal? (raw-value job.recipientEmail) "blocked@example.com") (reject "notifications blocked for recipient" #:http-code 500) (begin (publish-event! UserNotifications (format "~a" (raw-value job.recipientUserId)) (NotificationDelivered (raw-value job.recipientEmail) (raw-value job.subject))) job)))))

(define/pow
  (deadNotifyWorker [job : KanelNotifyJob ::: (FromDeadQueue (Id == jobId) job)])
  #:capabilities [notifyWorkerCap kanelPubSub]
  #:returns KanelNotifyJob
  (let ([_ (thsl-src! "example/kanel/KanelBackend.tesl" 201 (list (cons 'job *job)) (lambda () (publish-event! UserNotifications (format "~a" (raw-value job.recipientUserId)) (NotificationFailed (raw-value job.recipientEmail) (raw-value job.subject)))))]) (thsl-src! "example/kanel/KanelBackend.tesl" 202 (list (cons 'job *job)) (lambda () *job))))
