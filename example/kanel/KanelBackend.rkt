...
(define/pow
  ...
  (thsl-src! "example/kanel/KanelBackend.tesl" 185 (list (cons 'job *job)) (lambda () (if (equal? (raw-value job.recipientEmail) "blocked@example.com") (reject "notifications blocked for recipient" #:http-code 500) (begin (publish-event! UserNotifications (format "~a" (raw-value job.recipientUserId)) (NotificationDelivered (raw-value job.recipientEmail) (raw-value job.subject))) job)))))

(define/pow
  (deadNotifyWorker [job : KanelNotifyJob ::: (FromDeadQueue (Id == jobId) job)])
  #:capabilities [notifyWorkerCap kanelPubSub]
  #:returns KanelNotifyJob
  (let ([_ (thsl-src! "example/kanel/KanelBackend.tesl" 193 (list (cons 'job *job)) (lambda () (publish-event! UserNotifications (format "~a" (raw-value job.recipientUserId)) (NotificationFailed (raw-value job.recipientEmail) (raw-value job.subject)))))]) (thsl-src! "example/kanel/KanelBackend.tesl" 194 (list (cons 'job *job)) (lambda () *job))))
...
(define-handler
  ...
  (thsl-src! "example/kanel/KanelBackend.tesl" 395 (list (cons 'session *session) (cons 'orgId *orgId) (cons 'issueId *issueId) (cons 'req *req)) (lambda () (let/check ([tesl_checked_0 (checkOrgMember (raw-value session.userId) orgId)]) (let ([userId tesl_checked_0]) (let/check ([tesl_checked_1 (checkNotDone issueId)]) (let ([notDoneId tesl_checked_1]) (let/check ([tesl_checked_2 (checkCommentBody (tesl_import_String_trim (raw-value req.body)))]) (let ([commentBody tesl_checked_2]) (let ([commentId (generatePrefixedId "cmt")]) (let ([issue (let ([tesl_match (select-one (from Issue) (where (==. (entity-field-ref Issue 'id) issueId)) (where (==. (entity-field-ref Issue 'orgId) orgId)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_3 (raw-value issue)]) (cond [(and (adt-value? *tesl_case_3) (eq? (adt-value-variant *tesl_case_3) 'Nothing)) (thsl-src! "example/kanel/KanelBackend.tesl" 401 (list) (lambda () (reject "issue not found" #:http-code 404)))] [(and (adt-value? *tesl_case_3) (eq? (adt-value-variant *tesl_case_3) 'Something)) (let ([currentIssue (hash-ref (adt-value-fields *tesl_case_3) 'value)]) (thsl-src! "example/kanel/KanelBackend.tesl" 403 (list (cons 'currentIssue currentIssue)) (lambda () (let ([reporter (let ([tesl_match (select-one (from KanelUser) (where (==. (entity-field-ref KanelUser 'id) (raw-value currentIssue.reporterId))))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_4 (raw-value reporter)]) (cond [(and (adt-value? *tesl_case_4) (eq? (adt-value-variant *tesl_case_4) 'Nothing)) (thsl-src! "example/kanel/KanelBackend.tesl" 405 (list) (lambda () (reject "issue reporter not found" #:http-code 404)))] [(and (adt-value? *tesl_case_4) (eq? (adt-value-variant *tesl_case_4) 'Something)) (let ([targetUser (hash-ref (adt-value-fields *tesl_case_4) 'value)]) (thsl-src! "example/kanel/KanelBackend.tesl" 407 (list (cons 'targetUser targetUser)) (lambda () (call-with-queue-transaction (lambda () (begin (enqueue! _queue_for_KanelNotifyJob (KanelNotifyJob #:recipientUserId (raw-value targetUser.id) #:recipientEmail (raw-value targetUser.email) #:subject "New comment on issue {issueId}" #:body "Comment from {session.displayName}: {commentBody}")) (pack ([commentId]) (insertCommentBody commentId userId orgId notDoneId commentBody))))))))]))))))])))))))))))))
