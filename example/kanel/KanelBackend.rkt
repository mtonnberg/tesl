...
(define/pow
  ...
  #:capabilities [notifyWorkerCap kanelPubSub]
  #:returns KanelNotifyJob
  (let ([_ (thsl-src! "example/kanel/KanelBackend.tesl" 202 (list (cons 'job *job)) (lambda () (publish-event! UserNotifications (format "~a" (raw-value job.recipientUserId)) (NotificationFailed (raw-value job.recipientEmail) (raw-value job.subject)))))]) (thsl-src! "example/kanel/KanelBackend.tesl" 203 (list (cons 'job *job)) (lambda () *job))))
        ...
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (kanelDbRead)
              (define orgs (thsl-src! "example/kanel/KanelBackend.tesl" 454 (list) (lambda () (dispatch-api-test-request KanelServer 'get (list "orgs") #:headers (hash) #:capabilities (list kanelDbRead)))))
        ...
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (kanelDbRead kanelDbWrite random time)
              (define registered (thsl-src! "example/kanel/KanelBackend.tesl" 459 (list) (lambda () (dispatch-api-test-request KanelServer 'post (list "auth" "register") #:headers (hash) #:body (hash (string->symbol "email") "alice@example.com" (string->symbol "password") "password123" (string->symbol "displayName") "Alice") #:capabilities (list kanelDbRead kanelDbWrite random time)))))
              ...
              (define duplicate (thsl-src! "example/kanel/KanelBackend.tesl" 469 (list (cons 'registeredDisplayName registeredDisplayName) (cons 'registeredEmail registeredEmail) (cons 'registered registered)) (lambda () (dispatch-api-test-request KanelServer 'post (list "auth" "register") #:headers (hash) #:body (hash (string->symbol "email") (api-test-string-fragment (raw-value registeredEmail)) (string->symbol "password") "password123" (string->symbol "displayName") (api-test-string-fragment (raw-value registeredDisplayName))) #:capabilities (list kanelDbRead kanelDbWrite random time)))))
    ...
    (call-with-fresh-memory-db (list KanelDatabase)
      (lambda ()
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (kanelDbRead kanelDbWrite random time)
              (define registered (thsl-src! "example/kanel/KanelBackend.tesl" 478 (list) (lambda () (dispatch-api-test-request KanelServer 'post (list "auth" "register") #:headers (hash) #:body (hash (string->symbol "email") "bob@example.com" (string->symbol "password") "password123" (string->symbol "displayName") "Bob") #:capabilities (list kanelDbRead kanelDbWrite random time)))))
              ...
              (define login (thsl-src! "example/kanel/KanelBackend.tesl" 482 (list (cons 'registeredUserId registeredUserId) (cons 'registered registered)) (lambda () (dispatch-api-test-request KanelServer 'post (list "auth" "login") #:headers (hash) #:body (hash (string->symbol "email") "bob@example.com" (string->symbol "password") "password123") #:capabilities (list kanelDbRead kanelDbWrite random time)))))
              ...
              (define createdOrg (thsl-src! "example/kanel/KanelBackend.tesl" 487 (list (cons 'sessionUserId sessionUserId) (cons 'login login) (cons 'registeredUserId registeredUserId) (cons 'registered registered)) (lambda () (dispatch-api-test-request KanelServer 'post (list "orgs") #:cookie (string-append "kanel_user_id=" (api-test-string-fragment (raw-value sessionUserId))) #:headers (hash) #:body (hash (string->symbol "name") "Acme Inc" (string->symbol "slug") "acme-inc") #:capabilities (list kanelDbRead kanelDbWrite random time)))))
              ...
              (define orgs (thsl-src! "example/kanel/KanelBackend.tesl" 506 (list (cons 'projectId projectId) (cons 'createdProject createdProject) (cons 'orgSlug orgSlug) (cons 'orgSlugJson orgSlugJson) (cons 'orgId orgId) (cons 'createdOrg createdOrg) (cons 'sessionUserId sessionUserId) (cons 'login login) (cons 'registeredUserId registeredUserId) (cons 'registered registered)) (lambda () (dispatch-api-test-request KanelServer 'get (list "orgs") #:cookie (string-append "kanel_user_id=" (api-test-string-fragment (raw-value sessionUserId))) #:headers (hash) #:capabilities (list kanelDbRead kanelDbWrite random time)))))
        ...
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (kanelDbRead kanelDbWrite)
              ...
              (define orgs (thsl-src! "example/kanel/KanelBackend.tesl" 556 (list (cons 'orgId orgId) (cons 'userId userId)) (lambda () (dispatch-api-test-request KanelServer 'get (list "orgs") #:cookie (string-append "kanel_user_id=" (api-test-string-fragment (raw-value userId))) #:headers (hash) #:capabilities (list kanelDbRead kanelDbWrite)))))
        ...
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (kanelDbRead kanelDbWrite kanelQueue kanelPubSub notifyWorkerCap random time)
              ...
              (define stream (thsl-src! "example/kanel/KanelBackend.tesl" 633 (list (cons 'issueId issueId) (cons 'orgId orgId) (cons 'commenterId commenterId) (cons 'reporterId reporterId)) (lambda () (subscribe KanelServer-sse-routes (list "events" "users" (api-test-path-fragment (raw-value reporterId))) #:cookie (string-append "kanel_user_id=" (api-test-string-fragment (raw-value reporterId))) #:headers (hash) #:name "/events/users/{reporterId}"))))
              ...
              (check-true (raw-value (thsl-src! "example/kanel/KanelBackend.tesl" 649 (list (cons 'events events) (cons 'job job) (cons 'queued queued) (cons 'comment comment) (cons 'stream stream) (cons 'issueId issueId) (cons 'orgId orgId) (cons 'commenterId commenterId) (cons 'reporterId reporterId)) (lambda () (includesWhere (hash 'tag "NotificationDelivered" 'fields (hash 'recipientEmail "reporter@example.com")) (raw-value events))))))
            )
          ))
      ))
  )
        ...
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (kanelDbRead kanelDbWrite kanelQueue kanelPubSub notifyWorkerCap random time)
              ...
              (define stream (thsl-src! "example/kanel/KanelBackend.tesl" 717 (list (cons 'issueId issueId) (cons 'orgId orgId) (cons 'commenterId commenterId) (cons 'reporterId reporterId)) (lambda () (subscribe KanelServer-sse-routes (list "events" "users" (api-test-path-fragment (raw-value reporterId))) #:cookie (string-append "kanel_user_id=" (api-test-string-fragment (raw-value reporterId))) #:headers (hash) #:name "/events/users/{reporterId}"))))
              ...
              (check-true (raw-value (thsl-src! "example/kanel/KanelBackend.tesl" 745 (list (cons 'events events) (cons 'deadJob deadJob) (cons 'deadResult deadResult) (cons 'thirdError thirdError) (cons 'thirdAttempt thirdAttempt) (cons 'secondError secondError) (cons 'secondAttempt secondAttempt) (cons 'firstError firstError) (cons 'firstAttempt firstAttempt) (cons 'comment comment) (cons 'stream stream) (cons 'issueId issueId) (cons 'orgId orgId) (cons 'commenterId commenterId) (cons 'reporterId reporterId)) (lambda () (includesWhere (hash 'tag "NotificationFailed" 'fields (hash 'recipientEmail "blocked@example.com")) (raw-value events))))))
            )
          ))
      ))
  )
        ...
        (call-with-api-test-subscriptions
          (lambda ()
            (with-capabilities (kanelDbRead kanelDbWrite kanelPubSub time)
              ...
              (check-equal? (raw-value (thsl-src! "example/kanel/KanelBackend.tesl" 803 (list (cons 'toTodo toTodo) (cons 'issueId issueId) (cons 'orgId orgId) (cons 'userId userId)) (lambda () (api-test-field-access-ref (api-test-field-access-ref toTodo 'body) 'status)))) (hash 'tag "Todo"))
