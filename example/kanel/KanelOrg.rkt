...
(define-handler
  ...
  (thsl-src! "example/kanel/KanelOrg.tesl" 203 (list (cons 'session *session) (cons 'req *req)) (lambda () (let/check ([tesl-checked-5 (checkOrgName (tesl_import_String_trim (raw-value req.name)))]) (let ([orgName tesl-checked-5]) (let/check ([tesl-checked-6 (checkSlug (tesl_import_String_toLower (raw-value (tesl_import_String_trim (raw-value req.slug)))))]) (let ([slug tesl-checked-6]) (let ([existing (let ([tesl_match (select-one (from Org) (where (==. (entity-field-ref Org 'slug) slug)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl-case-7 (raw-value existing)]) (cond [(and (adt-value? *tesl-case-7) (eq? (adt-value-variant *tesl-case-7) 'Something)) (thsl-src! "example/kanel/KanelOrg.tesl" 208 (list) (lambda () (reject "slug already taken" #:http-code 409)))] [(and (adt-value? *tesl-case-7) (eq? (adt-value-variant *tesl-case-7) 'Nothing)) (thsl-src! "example/kanel/KanelOrg.tesl" 210 (list) (lambda () (let ([orgId (generatePrefixedId "org")]) (let ([memberId (generatePrefixedId "mem")]) (call-with-queue-transaction (lambda () (let ([_ (insert-one! OrgMembership (hash 'id memberId 'orgId orgId 'userId (raw-value session.userId) 'role RoleAdmin 'joinedAt (raw-value (nowMillis))))]) (pack ([orgId]) (insert-one! Org (hash 'id orgId 'name orgName 'slug slug 'createdAt (raw-value (nowMillis))))))))))))]))))))))))

(define-handler
  (getOrgHandler [session : KanelSession ::: (Authenticated session)] [orgId : String ::: (ValidOrgId orgId)])
  #:capabilities [kanelDbRead]
  #:returns Org
  (thsl-src! "example/kanel/KanelOrg.tesl" 236 (list (cons 'session *session) (cons 'orgId *orgId)) (lambda () (let/check ([tesl-checked-8 (checkOrgMember (raw-value session.userId) orgId)]) (let ([_member tesl-checked-8]) (let ([found (let ([tesl_match (select-one (from Org) (where (==. (entity-field-ref Org 'id) orgId)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl-case-9 (raw-value found)]) (cond [(and (adt-value? *tesl-case-9) (eq? (adt-value-variant *tesl-case-9) 'Nothing)) (thsl-src! "example/kanel/KanelOrg.tesl" 239 (list) (lambda () (reject "organization not found" #:http-code 404)))] [(and (adt-value? *tesl-case-9) (eq? (adt-value-variant *tesl-case-9) 'Something)) (let ([org (hash-ref (adt-value-fields *tesl-case-9) 'value)]) (thsl-src! "example/kanel/KanelOrg.tesl" 240 (list (cons 'org org)) (lambda () *org)))]))))))))

(define/pow
  (fetchOrgByMembership [acc : (List Org)] [m : OrgMembership])
  #:capabilities [kanelDbRead]
  #:returns (List Org)
  (let ([found (thsl-src! "example/kanel/KanelOrg.tesl" 246 (list (cons 'acc *acc) (cons 'm *m)) (lambda () (let ([tesl_match (select-one (from Org) (where (==. (entity-field-ref Org 'id) (tesl-dot/runtime m 'orgId 'OrgMembership))))]) (if tesl_match (Something tesl_match) Nothing))))]) (thsl-src-control! "example/kanel/KanelOrg.tesl" 247 (list (cons 'found *found) (cons 'acc *acc) (cons 'm *m)) (lambda () (let ([tesl-case-10 (raw-value found)]) (cond [(and (adt-value? *tesl-case-10) (eq? (adt-value-variant *tesl-case-10) 'Nothing)) (thsl-src! "example/kanel/KanelOrg.tesl" 248 (list) (lambda () *acc))] [(and (adt-value? *tesl-case-10) (eq? (adt-value-variant *tesl-case-10) 'Something)) (let ([o (hash-ref (adt-value-fields *tesl-case-10) 'value)]) (thsl-src! "example/kanel/KanelOrg.tesl" 249 (list (cons 'o o)) (lambda () (raw-value (tesl_import_List_append *acc (list *o))))))]))))))
...
(define-handler
  ...
  (thsl-src! "example/kanel/KanelOrg.tesl" 292 (list (cons 'session *session) (cons 'orgId *orgId)) (lambda () (let/check ([tesl-checked-11 (checkOrgMember (raw-value session.userId) orgId)]) (let ([_member tesl-checked-11]) (select-many (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'orgId) orgId))))))))
...
(define-handler
  ...
  #:capabilities [kanelDbRead kanelDbWrite]
  #:returns String
  (thsl-src! "example/kanel/KanelOrg.tesl" 363 (list (cons 'session *session) (cons 'orgId *orgId) (cons 'targetUserId *targetUserId)) (lambda () (let/check ([tesl-checked-19 (checkOrgAdmin (raw-value session.userId) orgId)]) (let ([_adminUserId tesl-checked-19]) (let ([targetMembership (let ([tesl_match (select-one (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl-case-20 (raw-value targetMembership)]) (cond [(and (adt-value? *tesl-case-20) (eq? (adt-value-variant *tesl-case-20) 'Nothing)) (thsl-src! "example/kanel/KanelOrg.tesl" 366 (list) (lambda () (reject "user is not a member of this organization" #:http-code 404)))] [(and (adt-value? *tesl-case-20) (eq? (adt-value-variant *tesl-case-20) 'Something)) (let ([membership (hash-ref (adt-value-fields *tesl-case-20) 'value)]) (thsl-src! "example/kanel/KanelOrg.tesl" 368 (list (cons 'membership membership)) (lambda () (let ([tesl-case-21 (tesl-dot/runtime membership 'role 'OrgMembership)]) (cond [(and (adt-value? *tesl-case-21) (eq? (adt-value-variant *tesl-case-21) 'RoleAdmin)) (thsl-src! "example/kanel/KanelOrg.tesl" 370 (list) (lambda () (let ([adminCount (select-count (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)) (where (==. (entity-field-ref OrgMembership 'role) RoleAdmin)))]) (if (<= *adminCount 1) (reject "cannot remove the last admin" #:http-code 400) (let ([_ (delete-many! (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) "removed")))))] [(and (adt-value? *tesl-case-21) (eq? (adt-value-variant *tesl-case-21) 'RoleMember)) (thsl-src! "example/kanel/KanelOrg.tesl" 377 (list) (lambda () (let ([_ (delete-many! (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) "removed")))] [(and (adt-value? *tesl-case-21) (eq? (adt-value-variant *tesl-case-21) 'RoleViewer)) (thsl-src! "example/kanel/KanelOrg.tesl" 380 (list) (lambda () (let ([_ (delete-many! (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) "removed")))])))))]))))))))
