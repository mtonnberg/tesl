...
(define (tesl-codec-decode-RegisterRequest-0 _j)
  (define _f_email (tesl-codec-decode-field _j "email" tesl-json-string-codec))
  (define _f_password (tesl-codec-decode-field _j "password" tesl-json-string-codec))
  (define _f_displayName (tesl-codec-decode-field _j "displayName" tesl-json-string-codec))
...
(define-handler
  ...
  (let/check ([tesl_checked_5 (checkOrgName (tesl_import_String_trim (raw-value req.name)))]) (let ([orgName tesl_checked_5]) (let/check ([tesl_checked_6 (checkSlug (tesl_import_String_toLower (raw-value (tesl_import_String_trim (raw-value req.slug)))))]) (let ([slug tesl_checked_6]) (let ([existing (let ([tesl_match (select-one (from Org) (where (==. (entity-field-ref Org 'slug) slug)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_7 (raw-value existing)]) (cond [(and (adt-value? *tesl_case_7) (eq? (adt-value-variant *tesl_case_7) 'Something)) (reject "slug already taken" #:http-code 409)] [(and (adt-value? *tesl_case_7) (eq? (adt-value-variant *tesl_case_7) 'Nothing)) (let ([orgId (generatePrefixedId "org")]) (let ([memberId (generatePrefixedId "mem")]) (call-with-queue-transaction (lambda () (let ([_ (insert-one! OrgMembership (hash 'id memberId 'orgId orgId 'userId (raw-value session.userId) 'role RoleAdmin 'joinedAt (raw-value (nowMillis))))]) (pack ([orgId]) (insert-one! Org (hash 'id orgId 'name orgName 'slug slug 'createdAt (raw-value (nowMillis))))))))))]))))))))

(define-handler
  (getOrgHandler [session : KanelSession ::: (Authenticated session)] [orgId : String ::: (ValidOrgId orgId)])
  #:capabilities [kanelDbRead]
  #:returns Org
  (let/check ([tesl_checked_8 (checkOrgMember (raw-value session.userId) orgId)]) (let ([_member tesl_checked_8]) (let ([found (let ([tesl_match (select-one (from Org) (where (==. (entity-field-ref Org 'id) orgId)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_9 (raw-value found)]) (cond [(and (adt-value? *tesl_case_9) (eq? (adt-value-variant *tesl_case_9) 'Nothing)) (reject "organization not found" #:http-code 404)] [(and (adt-value? *tesl_case_9) (eq? (adt-value-variant *tesl_case_9) 'Something)) (let ([org (hash-ref (adt-value-fields *tesl_case_9) 'value)]) *org)]))))))

(define/pow
  (fetchOrgByMembership [acc : (List Org)] [m : OrgMembership])
  #:capabilities [kanelDbRead]
  #:returns (List Org)
  (let ([found (let ([tesl_match (select-one (from Org) (where (==. (entity-field-ref Org 'id) (tesl-dot/runtime m 'orgId))))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_10 (raw-value found)]) (cond [(and (adt-value? *tesl_case_10) (eq? (adt-value-variant *tesl_case_10) 'Nothing)) *acc] [(and (adt-value? *tesl_case_10) (eq? (adt-value-variant *tesl_case_10) 'Something)) (let ([o (hash-ref (adt-value-fields *tesl_case_10) 'value)]) (raw-value (tesl_import_List_append *acc (list *o))))]))))

(define-handler
  (listMyOrgsHandler [session : KanelSession ::: (Authenticated session)])
  #:capabilities [kanelDbRead]
  #:returns (List Org)
  (let ([memberships (select-many (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) (raw-value session.userId))))]) (tesl_import_List_foldl fetchOrgByMembership (list) (raw-value memberships))))
...
(define-handler
  ...
  (let/check ([tesl_checked_11 (checkOrgMember (raw-value session.userId) orgId)]) (let ([_member tesl_checked_11]) (select-many (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'orgId) orgId))))))
...
(define-handler
  ...
  (let/check ([tesl_checked_12 (checkOrgAdmin (raw-value session.userId) orgId)]) (let ([_adminUserId tesl_checked_12]) (let/check ([tesl_checked_13 (checkEmail (tesl_import_String_trim (raw-value req.email)))]) (let ([email tesl_checked_13]) (let ([found (let ([tesl_match (select-one (from KanelUser) (where (==. (entity-field-ref KanelUser 'email) email)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_14 (raw-value found)]) (cond [(and (adt-value? *tesl_case_14) (eq? (adt-value-variant *tesl_case_14) 'Nothing)) (reject "user not found \u2014 they must register first" #:http-code 404)] [(and (adt-value? *tesl_case_14) (eq? (adt-value-variant *tesl_case_14) 'Something)) (let ([targetUser (hash-ref (adt-value-fields *tesl_case_14) 'value)]) (let ([existing (let ([tesl_match (select-one (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) (raw-value targetUser.id))) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_15 (raw-value existing)]) (cond [(and (adt-value? *tesl_case_15) (eq? (adt-value-variant *tesl_case_15) 'Something)) (reject "user is already a member" #:http-code 409)] [(and (adt-value? *tesl_case_15) (eq? (adt-value-variant *tesl_case_15) 'Nothing)) (let ([memberId (generatePrefixedId "mem")]) (pack ([memberId]) (insert-one! OrgMembership (hash 'id memberId 'orgId orgId 'userId (raw-value targetUser.id) 'role (raw-value req.role) 'joinedAt (raw-value (nowMillis))))))]))))]))))))))
...
(define-handler
  ...
  (let/check ([tesl_checked_16 (checkOrgAdmin (raw-value session.userId) orgId)]) (let ([_adminUserId tesl_checked_16]) (let ([allAdmins (select-many (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)) (where (==. (entity-field-ref OrgMembership 'role) RoleAdmin)))]) (let ([targetMembership (let ([tesl_match (select-one (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_17 (raw-value targetMembership)]) (cond [(and (adt-value? *tesl_case_17) (eq? (adt-value-variant *tesl_case_17) 'Nothing)) (reject "user is not a member of this organization" #:http-code 404)] [(and (adt-value? *tesl_case_17) (eq? (adt-value-variant *tesl_case_17) 'Something)) (let ([membership (hash-ref (adt-value-fields *tesl_case_17) 'value)]) (let ([tesl_case_18 (tesl-dot/runtime membership 'role)]) (cond [(and (adt-value? *tesl_case_18) (eq? (adt-value-variant *tesl_case_18) 'RoleAdmin)) (if (<= (raw-value (tesl_import_List_length (raw-value allAdmins))) 1) (reject "cannot demote the last admin \u2014 promote another member first" #:http-code 400) (car (update-many! (from OrgMembership) (hash (entity-field-ref OrgMembership 'role) (raw-value req.role)) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))))] [(and (adt-value? *tesl_case_18) (eq? (adt-value-variant *tesl_case_18) 'RoleMember)) (car (update-many! (from OrgMembership) (hash (entity-field-ref OrgMembership 'role) (raw-value req.role)) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId))))] [(and (adt-value? *tesl_case_18) (eq? (adt-value-variant *tesl_case_18) 'RoleViewer)) (car (update-many! (from OrgMembership) (hash (entity-field-ref OrgMembership 'role) (raw-value req.role)) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId))))])))])))))))
...
(define-handler
  ...
  #:capabilities [kanelDbRead kanelDbWrite]
  #:returns String
  (let/check ([tesl_checked_19 (checkOrgAdmin (raw-value session.userId) orgId)]) (let ([_adminUserId tesl_checked_19]) (let ([targetMembership (let ([tesl_match (select-one (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) (if tesl_match (Something tesl_match) Nothing))]) (let ([tesl_case_20 (raw-value targetMembership)]) (cond [(and (adt-value? *tesl_case_20) (eq? (adt-value-variant *tesl_case_20) 'Nothing)) (reject "user is not a member of this organization" #:http-code 404)] [(and (adt-value? *tesl_case_20) (eq? (adt-value-variant *tesl_case_20) 'Something)) (let ([membership (hash-ref (adt-value-fields *tesl_case_20) 'value)]) (let ([tesl_case_21 (tesl-dot/runtime membership 'role)]) (cond [(and (adt-value? *tesl_case_21) (eq? (adt-value-variant *tesl_case_21) 'RoleAdmin)) (let ([adminCount (select-count (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)) (where (==. (entity-field-ref OrgMembership 'role) RoleAdmin)))]) (if (<= (raw-value adminCount) 1) (reject "cannot remove the last admin" #:http-code 400) (let ([_ (delete-many! (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) "removed")))] [(and (adt-value? *tesl_case_21) (eq? (adt-value-variant *tesl_case_21) 'RoleMember)) (let ([_ (delete-many! (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) "removed")] [(and (adt-value? *tesl_case_21) (eq? (adt-value-variant *tesl_case_21) 'RoleViewer)) (let ([_ (delete-many! (from OrgMembership) (where (==. (entity-field-ref OrgMembership 'userId) targetUserId)) (where (==. (entity-field-ref OrgMembership 'orgId) orgId)))]) "removed")])))]))))))
