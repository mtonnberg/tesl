...
(define/pow
  (dostuff [x : Status2])
  #:returns Integer
  (thsl-src-control! "example/todo-api.tesl" 53 (list (cons 'x *x)) (lambda () (let ([tesl_case_0 *x]) (cond [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Opened)) (let ([s (hash-ref (adt-value-fields *tesl_case_0) 'value)]) (thsl-src! "example/todo-api.tesl" 54 (list (cons 's s)) (lambda () *s)))] [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Finished)) (thsl-src! "example/todo-api.tesl" 55 (list) (lambda () (raw-value 3)))])))))
...
(define/pow
  ...
  #:returns Integer
  (thsl-src-control! "example/todo-api.tesl" 105 (list (cons 'teslPort *teslPort) (cons 'portEnv *portEnv)) (lambda () (let ([tesl_case_2 *teslPort]) (cond [(and (adt-value? *tesl_case_2) (eq? (adt-value-variant *tesl_case_2) 'Something)) (let ([port (hash-ref (adt-value-fields *tesl_case_2) 'value)]) (thsl-src! "example/todo-api.tesl" 106 (list (cons 'port port)) (lambda () (raw-value (parsePortString *port "TESL_TODO_API_PORT")))))] [(and (adt-value? *tesl_case_2) (eq? (adt-value-variant *tesl_case_2) 'Nothing)) (thsl-src! "example/todo-api.tesl" 108 (list) (lambda () (let ([tesl_case_3 *portEnv]) (cond [(and (adt-value? *tesl_case_3) (eq? (adt-value-variant *tesl_case_3) 'Something)) (let ([port (hash-ref (adt-value-fields *tesl_case_3) 'value)]) (thsl-src! "example/todo-api.tesl" 109 (list (cons 'port port)) (lambda () (raw-value (parsePortString *port "PORT")))))] [(and (adt-value? *tesl_case_3) (eq? (adt-value-variant *tesl_case_3) 'Nothing)) (thsl-src! "example/todo-api.tesl" 110 (list) (lambda () (raw-value defaultExamplePort)))]))))])))))

(define-auther
  (cookieAuth [request : HttpRequest])
  #:capabilities [todoReadHttpCookie]
  #:returns [requestUser : User ::: (Authenticated requestUser)]
  (thsl-src-control! "example/todo-api.tesl" 116 (list (cons 'request *request)) (lambda () (let ([tesl_case_4 (raw-value (tesl_import_Dict_lookup "user" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl_case_4) (eq? (adt-value-variant *tesl_case_4) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl_case_4) 'value)]) (thsl-src! "example/todo-api.tesl" 117 (list (cons 'userId userId)) (lambda () (accept Authenticated #:value (User #:id *userId #:role "user")))))] [(and (adt-value? *tesl_case_4) (eq? (adt-value-variant *tesl_case_4) 'Nothing)) (thsl-src! "example/todo-api.tesl" 118 (list) (lambda () (reject "Missing or invalid user cookie" #:http-code 401)))])))))
...
(define-checker
  ...
  (thsl-src-control! "example/todo-api.tesl" 170 (list (cons 'todo *todo)) (lambda () (let ([tesl_case_5 (tesl-dot/runtime todo 'status)]) (cond [(and (adt-value? *tesl_case_5) (eq? (adt-value-variant *tesl_case_5) 'Open)) (thsl-src! "example/todo-api.tesl" 171 (list) (lambda () (accept (IsOpen todo) #:value *todo)))] [(and (adt-value? *tesl_case_5) (eq? (adt-value-variant *tesl_case_5) 'Done)) (thsl-src! "example/todo-api.tesl" 172 (list) (lambda () (reject "todo is already completed" #:http-code 422)))])))))
