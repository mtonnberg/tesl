...
(register-sql-read-lines! "example/todo-api.tesl" '(175 208 215 221 233))
...
(define/pow
  (dostuff [x : Status2])
  #:returns Integer
  (thsl-src-control! "example/todo-api.tesl" 52 (list (cons 'x *x)) (lambda () (let ([tesl-case-0 *x]) (cond [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Opened)) (let ([s (hash-ref (adt-value-fields *tesl-case-0) 'value)]) (thsl-src! "example/todo-api.tesl" 53 (list (cons 's s)) (lambda () *s)))] [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Finished)) (thsl-src! "example/todo-api.tesl" 54 (list) (lambda () (raw-value 3)))])))))
...
(define/pow
  ...
  #:capabilities [random]
  #:returns String
  (thsl-src! "example/todo-api.tesl" 82 (list) (lambda () (generatePrefixedId "todo"))))

(define-checker
  (isValidPort [port : Integer])
  #:returns [port : Integer ::: (ValidPort port)]
  (thsl-src! "example/todo-api.tesl" 87 (list (cons 'port *port)) (lambda () (if (and (<= 1 *port) (<= *port 65535)) (accept (ValidPort port) #:value *port) (reject "Port must be between 1 and 65535" #:http-code 400)))))

(define-trusted
  (validPort [port : Integer])
  #:returns (Maybe (Fact (ValidPort port)))
  (thsl-src! "example/todo-api.tesl" 93 (list (cons 'port *port)) (lambda () (if (and (<= 1 *port) (<= *port 65535)) (Something (trusted-proof (ValidPort port))) Nothing))))

(define/pow
  (parsePortString [rawPort : String] [source : String])
  #:returns (? Integer _entity ::: (ValidPort _entity))
  (thsl-src-control! "example/todo-api.tesl" 99 (list (cons 'rawPort *rawPort) (cons 'source *source)) (lambda () (let ([tesl-case-1 (raw-value (tesl_import_Int_parse *rawPort))]) (cond [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Something)) (let ([parsedPort (hash-ref (adt-value-fields *tesl-case-1) 'value)]) (thsl-src! "example/todo-api.tesl" 100 (list (cons 'parsedPort parsedPort)) (lambda () (isValidPort parsedPort))))] [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Nothing)) (thsl-src! "example/todo-api.tesl" 101 (list) (lambda () (reject (format "invalid ~a port value ~a; expected an integer between 1 and 65535" (tesl-display-val *source) (tesl-display-val *rawPort)) #:http-code 400)))])))))

(define/pow
  (resolveExamplePort [teslPort : (Maybe String)] [portEnv : (Maybe String)])
  #:returns Integer
  (thsl-src-control! "example/todo-api.tesl" 104 (list (cons 'teslPort *teslPort) (cons 'portEnv *portEnv)) (lambda () (let ([tesl-case-2 *teslPort]) (cond [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Something)) (let ([port (hash-ref (adt-value-fields *tesl-case-2) 'value)]) (thsl-src! "example/todo-api.tesl" 105 (list (cons 'port port)) (lambda () (raw-value (parsePortString *port "TESL_TODO_API_PORT")))))] [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Nothing)) (thsl-src! "example/todo-api.tesl" 107 (list) (lambda () (let ([tesl-case-3 *portEnv]) (cond [(and (adt-value? *tesl-case-3) (eq? (adt-value-variant *tesl-case-3) 'Something)) (let ([port (hash-ref (adt-value-fields *tesl-case-3) 'value)]) (thsl-src! "example/todo-api.tesl" 108 (list (cons 'port port)) (lambda () (raw-value (parsePortString *port "PORT")))))] [(and (adt-value? *tesl-case-3) (eq? (adt-value-variant *tesl-case-3) 'Nothing)) (thsl-src! "example/todo-api.tesl" 109 (list) (lambda () (raw-value defaultExamplePort)))]))))])))))

(define-auther
  (cookieAuth [request : HttpRequest])
  #:capabilities [todoReadHttpCookie]
  #:returns [requestUser : User ::: (Authenticated requestUser)]
  (thsl-src-control! "example/todo-api.tesl" 115 (list (cons 'request *request)) (lambda () (let ([tesl-case-4 (raw-value (tesl_import_Dict_lookup "user" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl-case-4) (eq? (adt-value-variant *tesl-case-4) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl-case-4) 'value)]) (thsl-src! "example/todo-api.tesl" 116 (list (cons 'userId userId)) (lambda () (accept Authenticated #:value (User #:id *userId #:role "user")))))] [(and (adt-value? *tesl-case-4) (eq? (adt-value-variant *tesl-case-4) 'Nothing)) (thsl-src! "example/todo-api.tesl" 117 (list) (lambda () (reject "Missing or invalid user cookie" #:http-code 401)))])))))

(define-checker
  (isSafeTitle [title : String])
  #:returns [title : String ::: (TitleSafe title)]
  (thsl-src! "example/todo-api.tesl" 122 (list (cons 'title *title)) (lambda () (if (and (<= 4 (raw-value (tesl_import_String_length *title))) (<= (raw-value (tesl_import_String_length *title)) 120)) (accept (TitleSafe title) #:value *title) (reject "Title must be between 3 and 120 characters" #:http-code 400)))))

(define-checker
  (lengthLessThan30 [title : String])
  #:returns [title : String ::: (LengthLessThan30 title)]
  (thsl-src! "example/todo-api.tesl" 130 (list (cons 'title *title)) (lambda () (if (< (raw-value (tesl_import_String_length *title)) 30) (accept (LengthLessThan30 title) #:value *title) (reject "Title must be be less than 30 characters" #:http-code 400)))))

(define-checker
  (containsAnA [title : String])
  #:returns [title : String ::: (ContainsAnA title)]
  (thsl-src! "example/todo-api.tesl" 138 (list (cons 'title *title)) (lambda () (if (tesl_import_String_contains *title "a") (accept (ContainsAnA title) #:value *title) (reject "Title must contain an a." #:http-code 400)))))
...
(define-checker
  ...
  (thsl-src! "example/todo-api.tesl" 159 (list (cons 'todoId *todoId)) (lambda () (if (and (raw-value (tesl_import_String_startsWith *todoId "todo-")) (> (raw-value (tesl_import_String_length *todoId)) 5)) (accept (TodoId todoId) #:value *todoId) (reject "Malformed todo id" #:http-code 400)))))
...
(define-checker
  ...
  (thsl-src-control! "example/todo-api.tesl" 169 (list (cons 'todo *todo)) (lambda () (let ([tesl-case-5 (tesl-dot/runtime todo 'status 'Todo)]) (cond [(and (adt-value? *tesl-case-5) (eq? (adt-value-variant *tesl-case-5) 'Open)) (thsl-src! "example/todo-api.tesl" 170 (list) (lambda () (accept (IsOpen todo) #:value *todo)))] [(and (adt-value? *tesl-case-5) (eq? (adt-value-variant *tesl-case-5) 'Done)) (thsl-src! "example/todo-api.tesl" 171 (list) (lambda () (reject "todo is already completed" #:http-code 422)))])))))

(define/pow
  (seedExampleData)
  #:capabilities [todoDbRead todoDbWrite time]
  #:returns Integer
  (thsl-src! "example/todo-api.tesl" 175 (list) (lambda () (if (raw-value (tesl_import_List_isEmpty (select-many (from Todo)))) (let ([_ (insert-one! Todo (hash 'id "todo-1" 'title "Review the SQL layer" 'ownerId (raw-value (UserId "mikael")) 'status Open 'createdAt (raw-value (nowMillis))))]) (let ([_ (insert-one! Todo (hash 'id "todo-2" 'title "Sketch more DSL examples" 'ownerId (raw-value (UserId "anna")) 'status Open 'createdAt (raw-value (nowMillis))))]) (raw-value 2))) (raw-value 0)))))

(define-handler
  (listTest [requestUser : User ::: (Authenticated requestUser)] [newTodos : (List String)])
  #:capabilities [todoDbRead todoDbWrite time random]
  #:returns String
  (thsl-src! "example/todo-api.tesl" 188 (list (cons 'requestUser *requestUser) (cons 'newTodos *newTodos)) (lambda () "hej")))
...
(define-handler
  ...
  (let ([todoId (thsl-src! "example/todo-api.tesl" 194 (list (cons 'requestUser *requestUser) (cons 'newTodo *newTodo)) (lambda () (generateTodoId)))]) (thsl-src! "example/todo-api.tesl" 195 (list (cons 'todoId *todoId) (cons 'requestUser *requestUser) (cons 'newTodo *newTodo)) (lambda () (pack ([todoId]) (insert-one! Todo (hash 'id todoId 'title (raw-value newTodo.title) 'ownerId (raw-value requestUser.id) 'status Open 'createdAt (raw-value (nowMillis)))))))))

(define-handler
  (listMyTodos [requestUser : User ::: (Authenticated requestUser)])
  #:capabilities [todoDbRead]
  #:returns (List Todo)
  (let ([_ (thsl-src! "example/todo-api.tesl" 207 (list (cons 'requestUser *requestUser)) (lambda () (telemetry-event! "todo.list" #:attributes (["user.id" (raw-value requestUser.id)]))))]) (thsl-src! "example/todo-api.tesl" 208 (list (cons 'requestUser *requestUser)) (lambda () (select-many (from Todo) (where (==. (entity-field-ref Todo 'ownerId) (raw-value requestUser.id))))))))

(define-handler
  (listOpenTodos [requestUser : User ::: (Authenticated requestUser)])
  #:capabilities [todoDbRead]
  #:returns (List Todo)
  (let ([myTodos (thsl-src! "example/todo-api.tesl" 215 (list (cons 'requestUser *requestUser)) (lambda () (select-many (from Todo) (where (==. (entity-field-ref Todo 'ownerId) (raw-value requestUser.id))))) 'myTodos)]) (thsl-src! "example/todo-api.tesl" 216 (list (cons 'myTodos *myTodos) (cons 'requestUser *requestUser)) (lambda () (tesl_import_List_filterCheck checkOpen (raw-value myTodos))))))
...
(define-handler
  ...
  (let ([existing (thsl-src! "example/todo-api.tesl" 221 (list (cons 'requestUser *requestUser) (cons 'todoId *todoId)) (lambda () (let ([tesl_match (select-one (from Todo) (where (==. (entity-field-ref Todo 'id) todoId)))]) (if tesl_match (Something tesl_match) Nothing))) 'existing)]) (thsl-src-control! "example/todo-api.tesl" 222 (list (cons 'existing *existing) (cons 'requestUser *requestUser) (cons 'todoId *todoId)) (lambda () (let ([tesl-case-6 (raw-value existing)]) (cond [(and (adt-value? *tesl-case-6) (eq? (adt-value-variant *tesl-case-6) 'Nothing)) (thsl-src! "example/todo-api.tesl" 224 (list) (lambda () (reject "Todo not found" #:http-code 404)))] [(and (and (adt-value? *tesl-case-6) (eq? (adt-value-variant *tesl-case-6) 'Something)) (let ([todo (hash-ref (adt-value-fields *tesl-case-6) 'value)]) (not (tesl-equal? (raw-value todo.ownerId) (raw-value requestUser.id))))) (let ([todo (hash-ref (adt-value-fields *tesl-case-6) 'value)]) (thsl-src! "example/todo-api.tesl" 226 (list (cons 'todo todo)) (lambda () (reject "Todo not owned by request user" #:http-code 403))))] [(and (adt-value? *tesl-case-6) (eq? (adt-value-variant *tesl-case-6) 'Something)) (let ([todo (hash-ref (adt-value-fields *tesl-case-6) 'value)]) (thsl-src! "example/todo-api.tesl" 228 (list (cons 'todo todo)) (lambda () todo)))]))))))

(define-handler
  (completeTodo [requestUser : User ::: (Authenticated requestUser)] [todoId : String ::: (TodoId todoId)])
  #:capabilities [todoDbRead todoDbWrite]
  #:returns (? Todo _entity ::: (FromDb (Id == todoId) _entity))
  (let ([existing (thsl-src! "example/todo-api.tesl" 233 (list (cons 'requestUser *requestUser) (cons 'todoId *todoId)) (lambda () (let ([tesl_match (select-one (from Todo) (where (==. (entity-field-ref Todo 'id) todoId)))]) (if tesl_match (Something tesl_match) Nothing))) 'existing)]) (thsl-src-control! "example/todo-api.tesl" 234 (list (cons 'existing *existing) (cons 'requestUser *requestUser) (cons 'todoId *todoId)) (lambda () (let ([tesl-case-7 (raw-value existing)]) (cond [(and (adt-value? *tesl-case-7) (eq? (adt-value-variant *tesl-case-7) 'Nothing)) (thsl-src! "example/todo-api.tesl" 236 (list) (lambda () (reject "Todo not found" #:http-code 404)))] [(and (and (adt-value? *tesl-case-7) (eq? (adt-value-variant *tesl-case-7) 'Something)) (let ([todo (hash-ref (adt-value-fields *tesl-case-7) 'value)]) (not (tesl-equal? (raw-value todo.ownerId) (raw-value requestUser.id))))) (let ([todo (hash-ref (adt-value-fields *tesl-case-7) 'value)]) (thsl-src! "example/todo-api.tesl" 238 (list (cons 'todo todo)) (lambda () (reject "Todo not owned by request user" #:http-code 403))))] [(and (adt-value? *tesl-case-7) (eq? (adt-value-variant *tesl-case-7) 'Something)) (thsl-src! "example/todo-api.tesl" 240 (list) (lambda () (car (update-many! (from Todo) (hash (entity-field-ref Todo 'status) Done) (where (==. (entity-field-ref Todo 'id) todoId))))))]))))))
