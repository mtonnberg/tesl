...
(define-auther
  ...
  (thsl-src-control! "example/admin-task-api.tesl" 53 (list (cons 'request *request)) (lambda () (let ([tesl_case_0 (raw-value (tesl_import_Dict_lookup "user" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Nothing)) (thsl-src! "example/admin-task-api.tesl" 54 (list) (lambda () (reject "Missing user cookie" #:http-code 401)))] [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl_case_0) 'value)]) (thsl-src! "example/admin-task-api.tesl" 56 (list (cons 'userId userId)) (lambda () (let ([tesl_case_1 (raw-value (tesl_import_Dict_lookup "role" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Something)) (let ([role (hash-ref (adt-value-fields *tesl_case_1) 'value)]) (thsl-src! "example/admin-task-api.tesl" 57 (list (cons 'role role)) (lambda () (accept Authenticated #:value (AdminUser #:id *userId #:role *role)))))] [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Nothing)) (thsl-src! "example/admin-task-api.tesl" 58 (list) (lambda () (accept Authenticated #:value (AdminUser #:id *userId #:role "user"))))])))))])))))
