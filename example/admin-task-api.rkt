...
(define-checker
  ...
  (thsl-src! "example/admin-task-api.tesl" 43 (list (cons 'taskId *taskId)) (lambda () (if (> *taskId 0) (accept (Positive taskId) #:value *taskId) (reject "Task id must be positive" #:http-code 400)))))

(define-auther
  (cookieUserAuth [request : HttpRequest])
  #:capabilities [readTaskCookie]
  #:returns [requestUser : AdminUser ::: (Authenticated requestUser)]
  (thsl-src-control! "example/admin-task-api.tesl" 52 (list (cons 'request *request)) (lambda () (let ([tesl-case-0 (raw-value (tesl_import_Dict_lookup "user" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Nothing)) (thsl-src! "example/admin-task-api.tesl" 53 (list) (lambda () (reject "Missing user cookie" #:http-code 401)))] [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl-case-0) 'value)]) (thsl-src! "example/admin-task-api.tesl" 55 (list (cons 'userId userId)) (lambda () (let ([tesl-case-1 (raw-value (tesl_import_Dict_lookup "role" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Something)) (let ([role (hash-ref (adt-value-fields *tesl-case-1) 'value)]) (thsl-src! "example/admin-task-api.tesl" 56 (list (cons 'role role)) (lambda () (accept Authenticated #:value (AdminUser #:id *userId #:role *role)))))] [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Nothing)) (thsl-src! "example/admin-task-api.tesl" 57 (list) (lambda () (accept Authenticated #:value (AdminUser #:id *userId #:role "user"))))])))))])))))
