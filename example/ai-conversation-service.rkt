...
(define/pow
  ...
  #:returns String
  (thsl-src-control! "example/ai-conversation-service.tesl" 83 (list (cons 'request *request)) (lambda () (let ([tesl_case_0 (raw-value (tesl_import_Dict_lookup "provider" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Something)) (let ([p (hash-ref (adt-value-fields *tesl_case_0) 'value)]) (thsl-src! "example/ai-conversation-service.tesl" 84 (list (cons 'p p)) (lambda () *p)))] [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Nothing)) (thsl-src! "example/ai-conversation-service.tesl" 85 (list) (lambda () (raw-value "claude")))])))))

(define/pow
  (keyOf [request : HttpRequest])
  #:returns String
  (thsl-src-control! "example/ai-conversation-service.tesl" 90 (list (cons 'request *request)) (lambda () (let ([tesl_case_1 (raw-value (tesl_import_Dict_lookup "x-llm-key" (raw-value request.headers)))]) (cond [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Something)) (let ([k (hash-ref (adt-value-fields *tesl_case_1) 'value)]) (thsl-src! "example/ai-conversation-service.tesl" 91 (list (cons 'k k)) (lambda () *k)))] [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Nothing)) (thsl-src! "example/ai-conversation-service.tesl" 92 (list) (lambda () (raw-value "")))])))))

(define-auther
  (consumerAuth [request : HttpRequest])
  #:capabilities [httpAuth]
  #:returns [requestUser : Consumer ::: (Authenticated requestUser)]
  (thsl-src-control! "example/ai-conversation-service.tesl" 96 (list (cons 'request *request)) (lambda () (let ([tesl_case_2 (raw-value (tesl_import_Dict_lookup "consumerId" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl_case_2) (eq? (adt-value-variant *tesl_case_2) 'Something)) (let ([cid (hash-ref (adt-value-fields *tesl_case_2) 'value)]) (thsl-src! "example/ai-conversation-service.tesl" 98 (list (cons 'cid cid)) (lambda () (accept Authenticated #:value (Consumer #:id *cid #:provider (providerOf request) #:apiKey (keyOf request))))))] [(and (adt-value? *tesl_case_2) (eq? (adt-value-variant *tesl_case_2) 'Nothing)) (thsl-src! "example/ai-conversation-service.tesl" 99 (list) (lambda () (reject "Missing consumer identity" #:http-code 401)))])))))
...
(define/pow
  ...
  (thsl-src-control! "example/ai-conversation-service.tesl" 193 (list (cons 'agent *agent) (cons 'requestUser *requestUser) (cons 'conversationId *conversationId)) (lambda () (let ([tesl_case_4 (raw-value (let ([tesl_match (select-one (from ConversationRecord) (where (==. (entity-field-ref ConversationRecord 'id) conversationId)) (where (==. (entity-field-ref ConversationRecord 'ownerId) (tesl-dot/runtime requestUser 'id))))]) (if tesl_match (Something tesl_match) Nothing)))]) (cond [(and (adt-value? *tesl_case_4) (eq? (adt-value-variant *tesl_case_4) 'Something)) (let ([r (hash-ref (adt-value-fields *tesl_case_4) 'value)]) (thsl-src! "example/ai-conversation-service.tesl" 194 (list (cons 'r r)) (lambda () (raw-value (raw-value (conversationFrom *agent (tesl-dot/runtime r 'transcript)))))))] [(and (adt-value? *tesl_case_4) (eq? (adt-value-variant *tesl_case_4) 'Nothing)) (thsl-src! "example/ai-conversation-service.tesl" 195 (list) (lambda () (raw-value (raw-value (newConversation *agent)))))])))))
...
(define/pow
  ...
  #:returns Unit
  (let ([json (thsl-src! "example/ai-conversation-service.tesl" 213 (list (cons 'requestUser *requestUser) (cons 'conversationId *conversationId) (cons 'conv *conv)) (lambda () (raw-value (conversationJson *conv))))]) (thsl-src! "example/ai-conversation-service.tesl" 214 (list (cons 'json *json) (cons 'requestUser *requestUser) (cons 'conversationId *conversationId) (cons 'conv *conv)) (lambda () (call-with-queue-transaction (lambda () (let ([tesl_case_5 (raw-value (let ([tesl_match (select-one (from ConversationRecord) (where (==. (entity-field-ref ConversationRecord 'id) conversationId)) (where (==. (entity-field-ref ConversationRecord 'ownerId) (tesl-dot/runtime requestUser 'id))))]) (if tesl_match (Something tesl_match) Nothing)))]) (cond [(and (adt-value? *tesl_case_5) (eq? (adt-value-variant *tesl_case_5) 'Something)) (let ([r (hash-ref (adt-value-fields *tesl_case_5) 'value)]) (thsl-src! "example/ai-conversation-service.tesl" 216 (list (cons 'r r)) (lambda () (raw-value (updateTranscript requestUser conversationId json)))))] [(and (adt-value? *tesl_case_5) (eq? (adt-value-variant *tesl_case_5) 'Nothing)) (thsl-src! "example/ai-conversation-service.tesl" 217 (list) (lambda () (raw-value (insertTranscript requestUser conversationId json))))]))))))))
...
(define ChatServer-sse-routes
  (list (list (list "chat" "events") consumerAuth ChatStream)))
(define-api ChatApi
  [sendMessage :
...
(module+ test
  ...
  (test-case "a consumer cannot access another consumer's conversation (isolation)"
    ...
    (define tesl_ignored_7 (thsl-src! "example/ai-conversation-service.tesl" 337 (list (cons 'bob bob) (cons 'alice alice) (cons 'agent agent)) (lambda () (replyTurn agent alice "shared-id" "remember my secret"))))
