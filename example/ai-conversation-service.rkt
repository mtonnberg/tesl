...
(define-capability httpAuth (implies jwt))
...
(define/pow
  ...
  #:returns String
  (thsl-src! "example/ai-conversation-service.tesl" 164 (list (cons 'orderId *orderId)) (lambda () (if (tesl-equal? *orderId "12") (raw-value "Under processing") (if (tesl-equal? *orderId "13") (raw-value "Shipped") (let ([tesl-case-5 (raw-value (let ([tesl_match (select-one (from Order) (where (==. (entity-field-ref Order 'id) orderId)))]) (if tesl_match (Something tesl_match) Nothing)))]) (cond [(and (adt-value? *tesl-case-5) (eq? (adt-value-variant *tesl-case-5) 'Something)) (let ([o (hash-ref (adt-value-fields *tesl-case-5) 'value)]) (thsl-src! "example/ai-conversation-service.tesl" 170 (list (cons 'o o)) (lambda () (raw-value (raw-value o.status)))))] [(and (adt-value? *tesl-case-5) (eq? (adt-value-variant *tesl-case-5) 'Nothing)) (thsl-src! "example/ai-conversation-service.tesl" 171 (list) (lambda () (raw-value "no such order")))])))))))
...
(define/pow
  ...
  #:returns Unit
  (let ([json (thsl-src! "example/ai-conversation-service.tesl" 234 (list (cons 'requestUser *requestUser) (cons 'conversationId *conversationId) (cons 'conv *conv)) (lambda () (raw-value (conversationJson *conv))))]) (thsl-src! "example/ai-conversation-service.tesl" 235 (list (cons 'json *json) (cons 'requestUser *requestUser) (cons 'conversationId *conversationId) (cons 'conv *conv)) (lambda () (call-with-queue-transaction (lambda () (let ([tesl-case-7 (raw-value (let ([tesl_match (select-one (from ConversationRecord) (where (==. (entity-field-ref ConversationRecord 'id) conversationId)) (where (==. (entity-field-ref ConversationRecord 'ownerId) (tesl-dot/runtime requestUser 'id 'Consumer))))]) (if tesl_match (Something tesl_match) Nothing)))]) (cond [(and (adt-value? *tesl-case-7) (eq? (adt-value-variant *tesl-case-7) 'Something)) (let ([r (hash-ref (adt-value-fields *tesl-case-7) 'value)]) (thsl-src! "example/ai-conversation-service.tesl" 237 (list (cons 'r r)) (lambda () (raw-value (updateTranscript requestUser conversationId json)))))] [(and (adt-value? *tesl-case-7) (eq? (adt-value-variant *tesl-case-7) 'Nothing)) (thsl-src! "example/ai-conversation-service.tesl" 238 (list) (lambda () (raw-value (insertTranscript requestUser conversationId json))))]))))))))
...
(define (tesl-codec-decode-MessageRequest-0 _j)
  ...
  (record-value 'MessageRequest (tesl-hash 'message _f_message)))
...
(module+ test
  ...
  (test-case "a consumer cannot access another consumer's conversation (isolation)"
    ...
    (define tesl-ignored-9 (thsl-src! "example/ai-conversation-service.tesl" 359 (list (cons 'bob bob) (cons 'alice alice) (cons 'agent agent)) (lambda () (replyTurn agent alice "shared-id" "remember my secret"))))
