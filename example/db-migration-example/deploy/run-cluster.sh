#!/usr/bin/env bash
# Two request nodes behind a reverse proxy, with writes throughout V1 -> V9.
set -euo pipefail
...
for revision in 1 2 3 4 5 6 7 8 9; do bash "$APP_DIR/deploy/build-revision.sh" "$revision" "$RUN_DIR/v$revision"; done
bash "$APP_DIR/deploy/build-frontend.sh"
bash "$APP_DIR/deploy/setup-local.sh"
bash "$APP_DIR/deploy/install-local.sh" "$RUN_DIR/v1/app"
python3 "$APP_DIR/deploy/cluster.py" "$RUN_DIR" "$APP_DIR/frontend" "${TODO_PROXY_PORT:-8085}"
