...
if len(sys.argv) != 3 or sys.argv[1] not in [str(n) for n in range(1, 10)]:
    sys.exit("usage: prepare-release.py REVISION(1..9) NEW_OUTPUT_DIRECTORY")
...
archive = json.loads((app / "deploy/releases" / f"v{revision}.json").read_text())
