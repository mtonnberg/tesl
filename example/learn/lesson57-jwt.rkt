...
(define/pow
  ...
  #:capabilities [authCap]
  #:returns String
  (thsl-src-control! "example/learn/lesson57-jwt.tesl" 104 (list (cons 'token *token) (cons 'secret *secret)) (lambda () (let ([tesl_case_0 (raw-value (tesl_import_Dict_lookup "sub" (raw-value (tesl_import_JWT_verify *token *secret))))]) (cond [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Nothing)) (thsl-src! "example/learn/lesson57-jwt.tesl" 105 (list) (lambda () (raw-value "")))] [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl_case_0) 'value)]) (thsl-src! "example/learn/lesson57-jwt.tesl" 106 (list (cons 'userId userId)) (lambda () *userId)))])))))

(define/pow
  (decodeToken [token : JwtToken])
  #:capabilities [authCap]
  #:returns String
  (thsl-src-control! "example/learn/lesson57-jwt.tesl" 118 (list (cons 'token *token)) (lambda () (let ([tesl_case_1 (raw-value (tesl_import_Dict_lookup "sub" (raw-value (tesl_import_JWT_decode *token))))]) (cond [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Nothing)) (thsl-src! "example/learn/lesson57-jwt.tesl" 119 (list) (lambda () (raw-value "")))] [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl_case_1) 'value)]) (thsl-src! "example/learn/lesson57-jwt.tesl" 120 (list (cons 'userId userId)) (lambda () *userId)))])))))

(define/pow
  (wrapAndVerify [rawToken : String] [rawSecret : String])
  #:capabilities [authCap]
  #:returns String
  (let ([token (thsl-src! "example/learn/lesson57-jwt.tesl" 133 (list (cons 'rawToken *rawToken) (cons 'rawSecret *rawSecret)) (lambda () (raw-value (JwtToken *rawToken))))]) (let ([secret (thsl-src! "example/learn/lesson57-jwt.tesl" 134 (list (cons 'token *token) (cons 'rawToken *rawToken) (cons 'rawSecret *rawSecret)) (lambda () (raw-value (JwtSecret *rawSecret))))]) (thsl-src-control! "example/learn/lesson57-jwt.tesl" 135 (list (cons 'secret *secret) (cons 'token *token) (cons 'rawToken *rawToken) (cons 'rawSecret *rawSecret)) (lambda () (let ([tesl_case_2 (raw-value (tesl_import_Dict_lookup "sub" (raw-value (tesl_import_JWT_verify (raw-value token) (raw-value secret)))))]) (cond [(and (adt-value? *tesl_case_2) (eq? (adt-value-variant *tesl_case_2) 'Nothing)) (thsl-src! "example/learn/lesson57-jwt.tesl" 136 (list) (lambda () (raw-value "")))] [(and (adt-value? *tesl_case_2) (eq? (adt-value-variant *tesl_case_2) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl_case_2) 'value)]) (thsl-src! "example/learn/lesson57-jwt.tesl" 137 (list (cons 'userId userId)) (lambda () *userId)))])))))))
