...
(define/pow
  ...
  #:returns EmailJob
  (let ([_ (thsl-src! "example/learn/lesson31-worker-concurrency.tesl" 117 (list (cons 'job *job)) (lambda () (telemetry-event! "email.sent" #:attributes (["recipient" (raw-value job.recipientId)] ["subject" (raw-value job.subject)]))))]) (thsl-src! "example/learn/lesson31-worker-concurrency.tesl" 118 (list (cons 'job *job)) (lambda () *job))))

(define/pow
  (handleDeadEmail [job : EmailJob ::: (FromDeadQueue (Id == jobId) job)])
  #:capabilities [deadEmailCap]
  #:returns EmailJob
  (let ([_ (thsl-src! "example/learn/lesson31-worker-concurrency.tesl" 129 (list (cons 'job *job)) (lambda () (telemetry-event! "email.dead" #:attributes (["recipient" (raw-value job.recipientId)] ["subject" (raw-value job.subject)]))))]) (thsl-src! "example/learn/lesson31-worker-concurrency.tesl" 130 (list (cons 'job *job)) (lambda () *job))))

(define-handler
  (sendWelcomeEmail)
  #:capabilities [enqueueEmail]
  #:returns String
  (let ([_ (thsl-src! "example/learn/lesson31-worker-concurrency.tesl" 137 (list) (lambda () (enqueue! EmailQueue (EmailJob #:recipientId "user-123" #:subject "Welcome!" #:body "Thanks for signing up."))))]) (thsl-src! "example/learn/lesson31-worker-concurrency.tesl" 138 (list) (lambda () "queued"))))
