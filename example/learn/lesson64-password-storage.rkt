...
(register-sql-read-lines! "example/learn/lesson64-password-storage.tesl" '(107 108))
...
(define/pow
  ...
  #:returns String
  (thsl-src! "example/learn/lesson64-password-storage.tesl" 186 (list (cons 'email *email) (cons 'verified *verified)) (lambda () *email)))

(define/pow
  (logIn [email : String] [submitted : String])
  #:capabilities [dbRead]
  #:returns String
  (thsl-src! "example/learn/lesson64-password-storage.tesl" 189 (list (cons 'email *email) (cons 'submitted *submitted)) (lambda () (let ([stored (passwordHashFor email)]) (let/check ([tesl-checked-2 (tesl_import_Crypto_checkPassword stored submitted)]) (let ([verified tesl-checked-2]) (raw-value (sessionFor email verified))))))))

(define/pow
  (needsStrongerHash [email : String])
  #:capabilities [dbRead]
  #:returns Boolean
  (thsl-src-control! "example/learn/lesson64-password-storage.tesl" 219 (list (cons 'email *email)) (lambda () (let ([tesl-case-3 (raw-value (passwordHashFor email))]) (cond [(and (adt-value? *tesl-case-3) (eq? (adt-value-variant *tesl-case-3) 'Nothing)) (thsl-src! "example/learn/lesson64-password-storage.tesl" 220 (list) (lambda () (raw-value #f)))] [(and (adt-value? *tesl-case-3) (eq? (adt-value-variant *tesl-case-3) 'Something)) (let ([hash (hash-ref (adt-value-fields *tesl-case-3) 'value)]) (thsl-src! "example/learn/lesson64-password-storage.tesl" 221 (list (cons 'hash hash)) (lambda () (raw-value (raw-value (tesl_import_Crypto_needsRehash *hash))))))])))))

(define/pow
  (freshResetToken)
  #:capabilities [accountWrite]
  #:returns String
  (thsl-src! "example/learn/lesson64-password-storage.tesl" 236 (list) (lambda () (raw-value (tesl_import_Crypto_randomToken)))))

(define/pow
  (resetTokenLookupKey [token : String])
  #:returns String
  (thsl-src! "example/learn/lesson64-password-storage.tesl" 239 (list (cons 'token *token)) (lambda () (raw-value (tesl_import_Crypto_fingerprint *token)))))
...
(module+ test
  ...
  (test-case "a registered password verifies"
    ...
    (define tesl-ignored-4 (thsl-src! "example/learn/lesson64-password-storage.tesl" 254 (list) (lambda () (registerAccount "u1" "ada@example.com" "correct horse battery staple"))))
    (check-equal? (raw-value (thsl-src! "example/learn/lesson64-password-storage.tesl" 255 (list) (lambda () (logIn "ada@example.com" "correct horse battery staple")))) "ada@example.com")
    )
    ))
  )
  ...
  (test-case "a hash minted with the current parameters does not need rehashing"
    (call-with-fresh-memory-db (list Accounts) (lambda ()
    (with-capabilities (accountWrite dbRead)
    (define tesl-ignored-5 (thsl-src! "example/learn/lesson64-password-storage.tesl" 259 (list) (lambda () (registerAccount "u2" "grace@example.com" "hopper"))))
    (check-equal? (raw-value (thsl-src! "example/learn/lesson64-password-storage.tesl" 260 (list) (lambda () (needsStrongerHash "grace@example.com")))) #f)
    )
    ))
  )
  ...
  (test-case "an unknown email reports no stored hash and needs no rehash"
    ...
    (check-equal? (raw-value (thsl-src! "example/learn/lesson64-password-storage.tesl" 264 (list) (lambda () (needsStrongerHash "nobody@example.com")))) #f)
    )
    ))
  )
  ...
  (test-case "change-password stores a hash of the NEW password"
    (call-with-fresh-memory-db (list Accounts) (lambda ()
    (with-capabilities (accountWrite dbRead)
    (define tesl-ignored-6 (thsl-src! "example/learn/lesson64-password-storage.tesl" 268 (list) (lambda () (registerAccount "u3" "linus@example.com" "old-one"))))
    (define tesl-ignored-7 (thsl-src! "example/learn/lesson64-password-storage.tesl" 269 (list) (lambda () (changePassword "linus@example.com" "old-one" "new-one"))))
    (check-equal? (raw-value (thsl-src! "example/learn/lesson64-password-storage.tesl" 272 (list) (lambda () (logIn "linus@example.com" "new-one")))) "linus@example.com")
    )
    ))
  )
  ...
  (test-case "a reset token is unguessable, URL-safe, and stored only as a fingerprint"
    ...
    (define token (thsl-src! "example/learn/lesson64-password-storage.tesl" 276 (list) (lambda () (freshResetToken))))
    (check-equal? (raw-value (thsl-src! "example/learn/lesson64-password-storage.tesl" 278 (list (cons 'token token)) (lambda () (tesl_import_String_length (raw-value token))))) 43)
    (check-equal? (raw-value (thsl-src! "example/learn/lesson64-password-storage.tesl" 279 (list (cons 'token token)) (lambda () (resetTokenLookupKey token)))) (raw-value (tesl_import_Crypto_fingerprint (raw-value token))))
    (define another (thsl-src! "example/learn/lesson64-password-storage.tesl" 281 (list (cons 'token token)) (lambda () (freshResetToken))))
    (check-equal? (raw-value (thsl-src! "example/learn/lesson64-password-storage.tesl" 282 (list (cons 'another another) (cons 'token token)) (lambda () (tesl-equal? (raw-value token) (raw-value another))))) #f)
    )
    ))
  )
