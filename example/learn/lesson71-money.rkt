...
(register-sql-read-lines! "example/learn/lesson71-money.tesl" '(124 204))
(define/pow
  (showPrice)
  #:returns String
...
(define/pow
  ...
  #:capabilities [dbRead]
  #:returns Money
  (thsl-src! "example/learn/lesson71-money.tesl" 124 (list) (lambda () (select-sum (entity-field-ref OrderLine 'price) (from OrderLine)))))

(define/pow
  (consultantInvoice [hourly : MoneyPerDuration] [worked : Real])
  #:returns Money
  (thsl-src! "example/learn/lesson71-money.tesl" 177 (list (cons 'hourly *hourly) (cons 'worked *worked)) (lambda () (__tmoney_tesl-money-rate-mul *hourly *worked))))
...
(module+ test
  ...
  (test-case "a Money column round-trips and selectSum sums one currency"
    ...
    (define tesl-ignored-3 (thsl-src! "example/learn/lesson71-money.tesl" 165 (list) (lambda () (insert-one! OrderLine (tesl-hash 'id "l1" 'price (raw-value (tesl_import_Money_usd 1050)) 'quantity 1)))))
    (define tesl-ignored-4 (thsl-src! "example/learn/lesson71-money.tesl" 166 (list) (lambda () (insert-one! OrderLine (tesl-hash 'id "l2" 'price (raw-value (tesl_import_Money_usd 500)) 'quantity 2)))))
    (check-equal? (raw-value (thsl-src! "example/learn/lesson71-money.tesl" 167 (list) (lambda () (raw-value (tesl_import_Money_display (raw-value (totalRevenue))))))) "$15.50")
    )
    ))
  )
  ...
  (test-case "950 SEK/h for 1.5 h bills 1425 SEK"
    (call-with-fresh-memory-db (list Shop Agency) (lambda ()
  (define hourly (thsl-src! "example/learn/lesson71-money.tesl" 180 (list) (lambda () (raw-value (tesl_import_MoneyRate_perHour (raw-value (tesl_import_Money_sek 95000)))))))
  (check-equal? (raw-value (thsl-src! "example/learn/lesson71-money.tesl" 181 (list (cons 'hourly hourly)) (lambda () (raw-value (tesl_import_MoneyRate_display (raw-value hourly)))))) "950.00 SEK/h")
  (check-equal? (raw-value (thsl-src! "example/learn/lesson71-money.tesl" 182 (list (cons 'hourly hourly)) (lambda () (raw-value (tesl_import_Money_display (raw-value (consultantInvoice hourly (raw-value (tesl_import_Duration_hours 1.5))))))))) "1425.00 SEK")
  (check-equal? (raw-value (thsl-src! "example/learn/lesson71-money.tesl" 183 (list (cons 'hourly hourly)) (lambda () (raw-value (tesl_import_Money_display (raw-value (consultantInvoice hourly (raw-value (tesl_import_Duration_minutes 30.))))))))) "475.00 SEK")
    ))
  )
  ...
  (test-case "a stored hourly rate round-trips and bills"
    (call-with-fresh-memory-db (list Shop Agency) (lambda ()
    (with-capabilities (dbRead dbWrite)
    (define tesl-ignored-5 (thsl-src! "example/learn/lesson71-money.tesl" 203 (list) (lambda () (insert-one! Consultant (tesl-hash 'id "c1" 'hourly (raw-value (tesl_import_MoneyRate_perHour (raw-value (tesl_import_Money_sek 95000)))))))))
    (define found (thsl-src! "example/learn/lesson71-money.tesl" 204 (list) (lambda () (let ([tesl_match (select-one (from Consultant) (where (==. (entity-field-ref Consultant 'id) "c1")))]) (if tesl_match (Something tesl_match) Nothing)))))
    (check-equal? (raw-value (thsl-src! "example/learn/lesson71-money.tesl" 205 (list (cons 'found found)) (lambda () (let ([*tesl-case-6 (raw-value found)]) (cond [(and (adt-value? *tesl-case-6) (eq? (adt-value-variant *tesl-case-6) 'Something)) (let ([c2 (hash-ref (adt-value-fields *tesl-case-6) 'value)]) (thsl-src! "example/learn/lesson71-money.tesl" 206 (list (cons 'c2 c2)) (lambda () (tesl-equal? (raw-value (tesl_import_Money_display (raw-value (consultantInvoice (raw-value (tesl-dot/runtime c2 'hourly 'Consultant)) (raw-value (tesl_import_Duration_hours 2.)))))) "1900.00 SEK"))))] [(and (adt-value? *tesl-case-6) (eq? (adt-value-variant *tesl-case-6) 'Nothing)) (thsl-src! "example/learn/lesson71-money.tesl" 207 (list) (lambda () #f))]))))) #t)
    )
    ))
  )
