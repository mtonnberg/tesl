...
(define-checker
  ...
  (thsl-src! "example/learn/lesson19-existential-witnesses.tesl" 67 (list (cons 's *s)) (lambda () (if (tesl-gt? (raw-value (tesl_import_String_length *s)) 8) (accept (IsTokenId s) #:value *s) (reject "invalid token id" #:http-code 400)))))
...
(define/pow
  ...
  (thsl-src! "example/learn/lesson19-existential-witnesses.tesl" 85 (list) (lambda () (let ([tokenId (generatePrefixedId "tok")]) (let/check ([tesl-checked-0 (checkTokenId tokenId)]) (let ([validated tesl-checked-0]) (pack ([tokenId]) validated)))))))
...
(define-checker
  ...
  (thsl-src! "example/learn/lesson19-existential-witnesses.tesl" 158 (list (cons 'session *session) (cons 'sessionId *sessionId) (cons 'user *user)) (lambda () (if (tesl-equal? (raw-value (tesl-dot/runtime session 'id 'Session)) *sessionId) (accept (IsCreatedSession (Id == sessionId) user) #:value *session) (reject "session id does not match the witness" #:http-code 500)))))
