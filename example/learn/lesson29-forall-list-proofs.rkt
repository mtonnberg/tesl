...
(define-checker
  ...
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 143 (list (cons 's *s)) (lambda () (if (and (>= (raw-value (tesl_import_String_length *s)) 1) (<= (raw-value (tesl_import_String_length *s)) 200)) (accept (ValidTitle s) #:value *s) (reject "title must be 1-200 characters" #:http-code 400)))))
...
(define-checker
  ...
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 166 (list (cons 's *s)) (lambda () (if (> (raw-value (tesl_import_String_length *s)) 5) (accept (ValidNoteId s) #:value *s) (reject "invalid note id" #:http-code 400)))))
...
(define-checker
  ...
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 178 (list (cons 'note *note)) (lambda () (if (equal? (raw-value note.active) "yes") (accept (IsActive note) #:value *note) (reject "note is not active" #:http-code 422)))))

(define-checker
  (checkPinned [note : Note])
  #:returns [note : Note ::: (IsPinned note)]
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 186 (list (cons 'note *note)) (lambda () (if (equal? (raw-value note.pinned) "yes") (accept (IsPinned note) #:value *note) (reject "note is not pinned" #:http-code 422)))))

(define-auther
  (cookieAuth [request : HttpRequest])
  #:capabilities [noteReadCookie]
  #:returns [user : String ::: (Authenticated user)]
  (thsl-src-control! "example/learn/lesson29-forall-list-proofs.tesl" 197 (list (cons 'request *request)) (lambda () (let ([tesl-case-0 (raw-value (tesl_import_Dict_lookup "user" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Nothing)) (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 198 (list) (lambda () (reject "not logged in" #:http-code 401)))] [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl-case-0) 'value)]) (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 199 (list (cons 'userId userId)) (lambda () (accept (Authenticated userId) #:value *userId))))])))))

(define-handler
  (listNotes [user : String ::: (Authenticated user)])
  #:capabilities [noteDbRead]
  #:returns (List Note)
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 206 (list (cons 'user *user)) (lambda () (select-many (from Note) (where (==. (entity-field-ref Note 'authorId) user))))))

(define-handler
  (listActiveNotes [user : String ::: (Authenticated user)])
  #:capabilities [noteDbRead]
  #:returns (List Note)
  (let ([allNotes (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 213 (list (cons 'user *user)) (lambda () (select-many (from Note) (where (==. (entity-field-ref Note 'authorId) user)))))]) (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 214 (list (cons 'allNotes *allNotes) (cons 'user *user)) (lambda () (tesl_import_List_filterCheck checkActive (raw-value allNotes))))))

(define-handler
  (listActivePinnedNotes [user : String ::: (Authenticated user)])
  #:capabilities [noteDbRead]
  #:returns (List Note)
  (let ([allNotes (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 224 (list (cons 'user *user)) (lambda () (select-many (from Note) (where (==. (entity-field-ref Note 'authorId) user)))))]) (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 225 (list (cons 'allNotes *allNotes) (cons 'user *user)) (lambda () (tesl_import_List_filterCheck (check-and checkActive checkPinned) (raw-value allNotes))))))

(define/pow
  (filterActivePinned [notes : (List Note)])
  #:returns (List Note)
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 235 (list (cons 'notes *notes)) (lambda () (tesl_import_List_filterCheck (check-and checkActive checkPinned) *notes))))

(define/pow
  (verifyAllActive [notes : (List Note)])
  #:returns (Maybe (List Note))
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 247 (list (cons 'notes *notes)) (lambda () (tesl_import_List_allCheck checkActive *notes))))

(define/pow
  (verifyAllActivePinned [notes : (List Note)])
  #:returns (Maybe (List Note))
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 256 (list (cons 'notes *notes)) (lambda () (tesl_import_List_allCheck (check-and checkActive checkPinned) *notes))))

(define/pow
  (countActivePinned [notes : (List Note)])
  #:returns Integer
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 264 (list (cons 'notes *notes)) (lambda () (raw-value (tesl_import_List_length *notes)))))

(define/pow
  (applyCombined [note : Note])
  #:returns (? Note _entity ::: ((IsActive _entity) && (IsPinned _entity)))
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 282 (list (cons 'note *note)) (lambda () ((check-and checkActive checkPinned) note))))
...
(define-handler
  ...
  (let ([noteId (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 290 (list (cons 'user *user) (cons 'body *body)) (lambda () (generatePrefixedId "note")))]) (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 291 (list (cons 'noteId *noteId) (cons 'user *user) (cons 'body *body)) (lambda () (pack ([noteId]) (insert-one! Note (hash 'id noteId 'title (raw-value body.title) 'content (raw-value body.content) 'authorId user 'active "yes" 'pinned "no")))))))
