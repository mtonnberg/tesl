...
(define-checker
  ...
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 140 (list (cons 's *s)) (lambda () (if (and (>= (raw-value (tesl_import_String_length *s)) 1) (<= (raw-value (tesl_import_String_length *s)) 200)) (accept (ValidTitle s) #:value *s) (reject "title must be 1-200 characters" #:http-code 400)))))
...
(define-checker
  ...
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 163 (list (cons 's *s)) (lambda () (if (> (raw-value (tesl_import_String_length *s)) 5) (accept (ValidNoteId s) #:value *s) (reject "invalid note id" #:http-code 400)))))
...
(define-checker
  ...
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 175 (list (cons 'note *note)) (lambda () (if (tesl-equal? (raw-value note.active) "yes") (accept (IsActive note) #:value *note) (reject "note is not active" #:http-code 422)))))

(define-checker
  (checkPinned [note : Note])
  #:returns [note : Note ::: (IsPinned note)]
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 183 (list (cons 'note *note)) (lambda () (if (tesl-equal? (raw-value note.pinned) "yes") (accept (IsPinned note) #:value *note) (reject "note is not pinned" #:http-code 422)))))

(define-auther
  (cookieAuth [request : HttpRequest])
  #:capabilities [noteReadCookie]
  #:returns [user : String ::: (Authenticated user)]
  (thsl-src-control! "example/learn/lesson29-forall-list-proofs.tesl" 194 (list (cons 'request *request)) (lambda () (let ([tesl-case-0 (raw-value (tesl_import_Dict_lookup "user" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Nothing)) (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 195 (list) (lambda () (reject "not logged in" #:http-code 401)))] [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl-case-0) 'value)]) (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 196 (list (cons 'userId userId)) (lambda () (accept (Authenticated userId) #:value *userId))))])))))

(define-handler
  (listNotes [user : String ::: (Authenticated user)])
  #:capabilities [noteDbRead]
  #:returns (List Note)
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 203 (list (cons 'user *user)) (lambda () (select-many (from Note) (where (==. (entity-field-ref Note 'authorId) user))))))
...
(define/pow
  ...
  #:returns (List Note)
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 232 (list (cons 'notes *notes)) (lambda () (tesl_import_List_filterCheck (check-and checkActive checkPinned) *notes))))

(define/pow
  (verifyAllActive [notes : (List Note)])
  #:returns (Maybe (List Note))
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 244 (list (cons 'notes *notes)) (lambda () (tesl_import_List_allCheck checkActive *notes))))

(define/pow
  (verifyAllActivePinned [notes : (List Note)])
  #:returns (Maybe (List Note))
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 253 (list (cons 'notes *notes)) (lambda () (tesl_import_List_allCheck (check-and checkActive checkPinned) *notes))))

(define/pow
  (countActivePinned [notes : (List Note)])
  #:returns Integer
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 261 (list (cons 'notes *notes)) (lambda () (raw-value (tesl_import_List_length *notes)))))

(define/pow
  (applyCombined [note : Note])
  #:returns (? Note _entity ::: ((IsActive _entity) && (IsPinned _entity)))
  (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 279 (list (cons 'note *note)) (lambda () ((check-and checkActive checkPinned) note))))
...
(define-handler
  ...
  (let ([noteId (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 287 (list (cons 'user *user) (cons 'body *body)) (lambda () (generatePrefixedId "note")))]) (thsl-src! "example/learn/lesson29-forall-list-proofs.tesl" 288 (list (cons 'noteId *noteId) (cons 'user *user) (cons 'body *body)) (lambda () (pack ([noteId]) (insert-one! Note (hash 'id noteId 'title (raw-value body.title) 'content (raw-value body.content) 'authorId user 'active "yes" 'pinned "no")))))))
