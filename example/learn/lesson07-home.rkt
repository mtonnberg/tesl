...
(define-checker
  ...
  (thsl-src! "example/learn/lesson07-home.tesl" 24 (list (cons 'n *n)) (lambda () (if (and (>= *n 0) (<= *n 1000)) (accept (InBounds n) #:value *n) (reject "out of bounds" #:http-code 400)))))

(define-checker
  (sanitize [s : String])
  #:returns [s : String ::: (Sanitized s)]
  (thsl-src! "example/learn/lesson07-home.tesl" 37 (list (cons 's *s)) (lambda () (if (<= (raw-value (tesl_import_String_length *s)) 256) (accept (Sanitized s) #:value *s) (reject "string too long" #:http-code 400)))))

(module+ test
  (require rackunit)
  (test-case "checkInBounds valid"
    (call-with-fresh-memory-db '() (lambda ()
  (define n1 (thsl-src! "example/learn/lesson07-home.tesl" 84 (list) (lambda () 0)))
  ...
  (define n2 (thsl-src! "example/learn/lesson07-home.tesl" 86 (list (cons 'r1 r1) (cons 'n1 n1)) (lambda () 500)))
  ...
  (define n3 (thsl-src! "example/learn/lesson07-home.tesl" 88 (list (cons 'r2 r2) (cons 'n2 n2) (cons 'r1 r1) (cons 'n1 n1)) (lambda () 1000)))
  ...
  (check-equal? (thsl-src! "example/learn/lesson07-home.tesl" 90 (list (cons 'r3 r3) (cons 'n3 n3) (cons 'r2 r2) (cons 'n2 n2) (cons 'r1 r1) (cons 'n1 n1)) (lambda () 1)) 1)
    ))
  )

  (test-case "checkInBounds rejects out-of-range"
    (call-with-fresh-memory-db '() (lambda ()
  (let ([tesl-ef-result (with-handlers ([exn:fail? (lambda (e) 'tesl-exception)]) (thsl-src! "example/learn/lesson07-home.tesl" 94 (list) (lambda ()
                          (checkInBounds -1))))])
  ...
  (test-case "sanitize valid"
    (call-with-fresh-memory-db '() (lambda ()
  (define s1 (thsl-src! "example/learn/lesson07-home.tesl" 99 (list) (lambda () "")))
  ...
  (define s2 (thsl-src! "example/learn/lesson07-home.tesl" 101 (list (cons 'r1 r1) (cons 's1 s1)) (lambda () "hello")))
  ...
  (check-equal? (thsl-src! "example/learn/lesson07-home.tesl" 103 (list (cons 'r2 r2) (cons 's2 s2) (cons 'r1 r1) (cons 's1 s1)) (lambda () 1)) 1)
    ))
  )

  (test-case "sanitize rejects too-long"
    (call-with-fresh-memory-db '() (lambda ()
  (let ([tesl-ef-result (with-handlers ([exn:fail? (lambda (e) 'tesl-exception)]) (thsl-src! "example/learn/lesson07-home.tesl" 107 (list) (lambda ()
