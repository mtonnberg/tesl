...
(define-auther
  ...
  (thsl-src-control! "example/learn/lesson06-proof-check-proof-auth.tesl" 87 (list (cons 'request *request)) (lambda () (let ([tesl_case_0 (raw-value (tesl_import_Dict_lookup "user" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Nothing)) (thsl-src! "example/learn/lesson06-proof-check-proof-auth.tesl" 88 (list) (lambda () (reject "admin only" #:http-code 401)))] [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl_case_0) 'value)]) (thsl-src! "example/learn/lesson06-proof-check-proof-auth.tesl" 90 (list (cons 'userId userId)) (lambda () (if (equal? (raw-value userId) "admin") (accept (IsAdmin userId) #:value *userId) (reject "admin only" #:http-code 401)))))])))))
