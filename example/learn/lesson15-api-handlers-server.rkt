...
(define-checker
  ...
  (thsl-src! "example/learn/lesson15-api-handlers-server.tesl" 75 (list (cons 'p *p)) (lambda () (if (and (>= *p 1) (<= *p 5)) (accept (ValidPriority p) #:value *p) (reject "priority must be 1-5" #:http-code 400)))))

(define-auther
  (cookieAuth [request : HttpRequest])
  #:returns [user : String ::: (Authenticated user)]
  (thsl-src-control! "example/learn/lesson15-api-handlers-server.tesl" 88 (list (cons 'request *request)) (lambda () (let ([tesl-case-0 (raw-value (tesl_import_Dict_lookup "user" (raw-value request.cookies)))]) (cond [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Nothing)) (thsl-src! "example/learn/lesson15-api-handlers-server.tesl" 89 (list) (lambda () (reject "not authenticated" #:http-code 401)))] [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Something)) (let ([userId (hash-ref (adt-value-fields *tesl-case-0) 'value)]) (thsl-src! "example/learn/lesson15-api-handlers-server.tesl" 90 (list (cons 'userId userId)) (lambda () (accept (Authenticated userId) #:value *userId))))])))))
...
(define-handler
  ...
  #:returns Task
  (thsl-src! "example/learn/lesson15-api-handlers-server.tesl" 106 (list (cons 'user *user) (cons 'body *body)) (lambda () (Task #:id "task-1" #:title (raw-value body.title) #:priority (raw-value body.priority) #:done #f))))

(define-handler
  (getTask [user : String ::: (Authenticated user)] [id : String])
  #:capabilities [taskDbRead]
  #:returns (Maybe Task)
  (thsl-src! "example/learn/lesson15-api-handlers-server.tesl" 114 (list (cons 'user *user) (cons 'id *id)) (lambda () (raw-value (Something (Task #:id *id #:title "example task" #:priority 3 #:done #f))))))
