...
(define WeatherAgent
  (__tart_withTools (__tart_defineAgent (raw-value (anthropic (raw-value (requireEnv "ANTHROPIC_API_KEY")) "claude-opus-4-8")) (raw-value "You are a helpful concierge. Use the tools to answer.") (raw-value 512)) (list (__tart_tool "getWeather" "Look up the current weather for a city." "{\"type\":\"object\",\"properties\":{\"city\":{\"type\":\"string\"}},\"required\":[\"city\"]}" (lambda (_args) (__tart_tesl-agent-decode-args _args (list (cons "city" 'string)))) (lambda (_decoded) (apply getWeather _decoded))) (__tart_tool "bookTable" "Book a table for a number of guests at a restaurant." "{\"type\":\"object\",\"properties\":{\"restaurant\":{\"type\":\"string\"},\"guests\":{\"type\":\"integer\"}},\"required\":[\"restaurant\",\"guests\"]}" (lambda (_args) (__tart_tesl-agent-decode-args _args (list (cons "restaurant" 'string) (cons "guests" 'int)))) (lambda (_decoded) (apply bookTable _decoded))))))
