...
(define/pow
  ...
  (let ([r (thsl-src! "example/learn/lesson20-named-db-results.tesl" 106 (list (cons 'id *id)) (lambda () (let ([tesl_match (select-one (from Task) (where (==. (entity-field-ref Task 'id) id)))]) (if tesl_match (Something tesl_match) Nothing))))]) (thsl-src! "example/learn/lesson20-named-db-results.tesl" 107 (list (cons 'r *r) (cons 'id *id)) (lambda () (let ([tesl_case_0 (raw-value r)]) (cond [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Nothing)) (reject "task not found" #:http-code 404)] [(and (adt-value? *tesl_case_0) (eq? (adt-value-variant *tesl_case_0) 'Something)) (let ([t (hash-ref (adt-value-fields *tesl_case_0) 'value)]) t)]))))))

(define/pow
  (seedAndFetch [id : String])
  #:capabilities [dbRead dbWrite]
  #:returns (? Task _entity ::: (FromDb (Id == id) _entity))
  (let ([_ (thsl-src! "example/learn/lesson20-named-db-results.tesl" 114 (list (cons 'id *id)) (lambda () (insert-one! Task (hash 'id id 'title (format "task: ~a" (tesl-display-val *id)) 'status "open"))))]) (let ([r (thsl-src! "example/learn/lesson20-named-db-results.tesl" 115 (list (cons '_ *_) (cons 'id *id)) (lambda () (let ([tesl_match (select-one (from Task) (where (==. (entity-field-ref Task 'id) id)))]) (if tesl_match (Something tesl_match) Nothing))))]) (thsl-src! "example/learn/lesson20-named-db-results.tesl" 116 (list (cons 'r *r) (cons '_ *_) (cons 'id *id)) (lambda () (let ([tesl_case_1 (raw-value r)]) (cond [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Nothing)) (reject "missing after insert" #:http-code 500)] [(and (adt-value? *tesl_case_1) (eq? (adt-value-variant *tesl_case_1) 'Something)) (let ([t (hash-ref (adt-value-fields *tesl_case_1) 'value)]) t)])))))))
