...
(define/pow
  ...
  (let ([r (thsl-src! "example/learn/lesson20-named-db-results.tesl" 106 (list (cons 'id *id)) (lambda () (let ([tesl_match (select-one (from Task) (where (==. (entity-field-ref Task 'id) id)))]) (if tesl_match (Something tesl_match) Nothing))))]) (thsl-src-control! "example/learn/lesson20-named-db-results.tesl" 107 (list (cons 'r *r) (cons 'id *id)) (lambda () (let ([tesl-case-0 (raw-value r)]) (cond [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Nothing)) (thsl-src! "example/learn/lesson20-named-db-results.tesl" 108 (list) (lambda () (reject "task not found" #:http-code 404)))] [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Something)) (let ([t (hash-ref (adt-value-fields *tesl-case-0) 'value)]) (thsl-src! "example/learn/lesson20-named-db-results.tesl" 109 (list (cons 't t)) (lambda () t)))]))))))

(define/pow
  (seedAndFetch [id : String])
  #:capabilities [dbRead dbWrite]
  #:returns (? Task _entity ::: (FromDb (Id == id) _entity))
  (let ([_ (thsl-src! "example/learn/lesson20-named-db-results.tesl" 114 (list (cons 'id *id)) (lambda () (insert-one! Task (hash 'id id 'title (format "task: ~a" (tesl-display-val *id)) 'status "open"))))]) (let ([r (thsl-src! "example/learn/lesson20-named-db-results.tesl" 115 (list (cons '_ *_) (cons 'id *id)) (lambda () (let ([tesl_match (select-one (from Task) (where (==. (entity-field-ref Task 'id) id)))]) (if tesl_match (Something tesl_match) Nothing))))]) (thsl-src-control! "example/learn/lesson20-named-db-results.tesl" 116 (list (cons 'r *r) (cons '_ *_) (cons 'id *id)) (lambda () (let ([tesl-case-1 (raw-value r)]) (cond [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Nothing)) (thsl-src! "example/learn/lesson20-named-db-results.tesl" 117 (list) (lambda () (reject "missing after insert" #:http-code 500)))] [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Something)) (let ([t (hash-ref (adt-value-fields *tesl-case-1) 'value)]) (thsl-src! "example/learn/lesson20-named-db-results.tesl" 118 (list (cons 't t)) (lambda () t)))])))))))
...
(module+ test
  ...
  (test-case "named db result preserves proof"
    ...
    (check-equal? (thsl-src! "example/learn/lesson20-named-db-results.tesl" 172 (list (cons 't t)) (lambda () (raw-value (tesl-dot/runtime t 'title 'Task)))) "task: test-1")
    (check-equal? (thsl-src! "example/learn/lesson20-named-db-results.tesl" 173 (list (cons 't t)) (lambda () (raw-value (tesl-dot/runtime t 'status 'Task)))) "open")
    )
    ))
  )
  ...
  (test-case "proof annotation verifies entity came from db"
    ...
    (check-equal? (thsl-src! "example/learn/lesson20-named-db-results.tesl" 182 (list (cons 't t) (cons 'queryId queryId)) (lambda () (raw-value (tesl-dot/runtime t 'title 'Task)))) "task: test-proof")
    )
    ))
  )
