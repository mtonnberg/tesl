...
(define/pow
  ...
  (thsl-src! "example/learn/lesson27-either-dict-set.tesl" 104 (list (cons 'raw *raw)) (lambda () (if (tesl_import_String_isEmpty *raw) (raw-value (raw-value (Left "age cannot be empty"))) (let ([tesl-case-0 (raw-value (tesl_import_String_toInt *raw))]) (cond [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Nothing)) (thsl-src! "example/learn/lesson27-either-dict-set.tesl" 108 (list) (lambda () (raw-value (raw-value (Left "age must be a number")))))] [(and (adt-value? *tesl-case-0) (eq? (adt-value-variant *tesl-case-0) 'Something)) (let ([n (hash-ref (adt-value-fields *tesl-case-0) 'value)]) (thsl-src! "example/learn/lesson27-either-dict-set.tesl" 110 (list (cons 'n n)) (lambda () (if (< *n 0) (raw-value (raw-value (Left "age cannot be negative"))) (if (> *n 150) (raw-value (raw-value (Left "age seems unrealistic"))) (raw-value (raw-value (Right *n))))))))]))))))
...
(define/pow
  ...
  #:returns (List String)
  (let ([perms (thsl-src! "example/learn/lesson27-either-dict-set.tesl" 153 (list (cons 'role *role)) (lambda () (raw-value (tesl_import_Dict_fromList (list (Tuple2 "admin" "read,write,delete,manage") (Tuple2 "member" "read,write") (Tuple2 "guest" "read"))))))]) (thsl-src-control! "example/learn/lesson27-either-dict-set.tesl" 154 (list (cons 'perms *perms) (cons 'role *role)) (lambda () (let ([tesl-case-1 (raw-value (tesl_import_Dict_lookup *role (raw-value perms)))]) (cond [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Nothing)) (thsl-src! "example/learn/lesson27-either-dict-set.tesl" 155 (list) (lambda () (raw-value (list))))] [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Something)) (let ([p (hash-ref (adt-value-fields *tesl-case-1) 'value)]) (thsl-src! "example/learn/lesson27-either-dict-set.tesl" 156 (list (cons 'p p)) (lambda () (raw-value (list *p)))))]))))))

(define/pow
  (currentCount [acc : (Dict String Integer)] [status : String])
  #:returns Integer
  (thsl-src-control! "example/learn/lesson27-either-dict-set.tesl" 162 (list (cons 'acc *acc) (cons 'status *status)) (lambda () (let ([tesl-case-2 (raw-value (tesl_import_Dict_lookup *status *acc))]) (cond [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Something)) (let ([value (hash-ref (adt-value-fields *tesl-case-2) 'value)]) (thsl-src! "example/learn/lesson27-either-dict-set.tesl" 163 (list (cons 'value value)) (lambda () *value)))] [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Nothing)) (thsl-src! "example/learn/lesson27-either-dict-set.tesl" 164 (list) (lambda () (raw-value 0)))])))))
