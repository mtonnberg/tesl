...
(module+ test
  ...
  (test-case "askFor decodes a typed Priority from the model's JSON"
    ...
    (check-equal? (thsl-src! "example/learn/lesson63-ai-structured-output.tesl" 94 (list (cons 'p p) (cons 'mock mock)) (lambda () (raw-value (tesl-dot/runtime p 'level 'Priority)))) 1)
    (check-equal? (thsl-src! "example/learn/lesson63-ai-structured-output.tesl" 95 (list (cons 'p p) (cons 'mock mock)) (lambda () (raw-value (tesl-dot/runtime p 'reason 'Priority)))) "production outage")
    )
    ))
  )
  ...
  (test-case "askFor retries past a malformed reply, then decodes"
    ...
    (check-equal? (thsl-src! "example/learn/lesson63-ai-structured-output.tesl" 103 (list (cons 'p p) (cons 'mock mock)) (lambda () (raw-value (tesl-dot/runtime p 'level 'Priority)))) 3)
    (check-equal? (thsl-src! "example/learn/lesson63-ai-structured-output.tesl" 104 (list (cons 'p p) (cons 'mock mock)) (lambda () (raw-value (tesl-dot/runtime p 'reason 'Priority)))) "cosmetic typo")
    )
    ))
  )
