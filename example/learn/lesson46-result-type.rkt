...
(define/pow
  ...
  #:returns Integer
  (thsl-src-control! "example/learn/lesson46-result-type.tesl" 69 (list (cons 'a *a) (cons 'b *b) (cons 'fallback *fallback)) (lambda () (let ([tesl-case-1 (raw-value (divide a b))]) (cond [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Ok)) (let ([n (hash-ref (adt-value-fields *tesl-case-1) 'value)]) (thsl-src! "example/learn/lesson46-result-type.tesl" 70 (list (cons 'n n)) (lambda () *n)))] [(and (adt-value? *tesl-case-1) (eq? (adt-value-variant *tesl-case-1) 'Err)) (thsl-src! "example/learn/lesson46-result-type.tesl" 71 (list) (lambda () *fallback))])))))

(define/pow
  (okOrDefault [r : (Result Integer String)] [default : Integer])
  #:returns Integer
  (thsl-src-control! "example/learn/lesson46-result-type.tesl" 75 (list (cons 'r *r) (cons 'default *default)) (lambda () (let ([tesl-case-2 *r]) (cond [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Ok)) (let ([n (hash-ref (adt-value-fields *tesl-case-2) 'value)]) (thsl-src! "example/learn/lesson46-result-type.tesl" 76 (list (cons 'n n)) (lambda () *n)))] [(and (adt-value? *tesl-case-2) (eq? (adt-value-variant *tesl-case-2) 'Err)) (thsl-src! "example/learn/lesson46-result-type.tesl" 77 (list) (lambda () *default))])))))

(define/pow
  (isErr [r : (Result Integer String)])
  #:returns Boolean
  (thsl-src-control! "example/learn/lesson46-result-type.tesl" 81 (list (cons 'r *r)) (lambda () (let ([tesl-case-3 *r]) (cond [(and (adt-value? *tesl-case-3) (eq? (adt-value-variant *tesl-case-3) 'Ok)) (thsl-src! "example/learn/lesson46-result-type.tesl" 82 (list) (lambda () (raw-value #f)))] [(and (adt-value? *tesl-case-3) (eq? (adt-value-variant *tesl-case-3) 'Err)) (thsl-src! "example/learn/lesson46-result-type.tesl" 83 (list) (lambda () (raw-value #t)))])))))
...
(define/pow
  ...
  (thsl-src-control! "example/learn/lesson46-result-type.tesl" 146 (list (cons 'rawAge *rawAge) (cons 'rawId *rawId)) (lambda () (let ([tesl-case-7 (raw-value (processAge rawAge))]) (cond [(and (adt-value? *tesl-case-7) (eq? (adt-value-variant *tesl-case-7) 'Err)) (let ([e (hash-ref (adt-value-fields *tesl-case-7) 'error)]) (thsl-src! "example/learn/lesson46-result-type.tesl" 147 (list (cons 'e e)) (lambda () (raw-value (raw-value (Err *e))))))] [(and (adt-value? *tesl-case-7) (eq? (adt-value-variant *tesl-case-7) 'Ok)) (let ([age (hash-ref (adt-value-fields *tesl-case-7) 'value)]) (thsl-src! "example/learn/lesson46-result-type.tesl" 149 (list (cons 'age age)) (lambda () (if (< *age 18) (raw-value (raw-value (Err "must be 18 or older"))) (if (tesl_import_String_isEmpty *rawId) (raw-value (raw-value (Err "id is required"))) (raw-value (raw-value (Ok (format "user ~a (age ~a) is valid" (tesl-display-val *rawId) (tesl-display-val (raw-value (tesl_import_Int_toString *age))))))))))))])))))

(define/pow
  (okIntOrDefault [r : (Result Integer String)] [d : Integer])
  #:returns Integer
  (thsl-src-control! "example/learn/lesson46-result-type.tesl" 161 (list (cons 'r *r) (cons 'd *d)) (lambda () (let ([tesl-case-8 *r]) (cond [(and (adt-value? *tesl-case-8) (eq? (adt-value-variant *tesl-case-8) 'Ok)) (let ([n (hash-ref (adt-value-fields *tesl-case-8) 'value)]) (thsl-src! "example/learn/lesson46-result-type.tesl" 162 (list (cons 'n n)) (lambda () *n)))] [(and (adt-value? *tesl-case-8) (eq? (adt-value-variant *tesl-case-8) 'Err)) (thsl-src! "example/learn/lesson46-result-type.tesl" 163 (list) (lambda () *d))])))))

(define/pow
  (errMsgOrEmpty [r : (Result Integer String)])
  #:returns String
  (thsl-src-control! "example/learn/lesson46-result-type.tesl" 166 (list (cons 'r *r)) (lambda () (let ([tesl-case-9 *r]) (cond [(and (adt-value? *tesl-case-9) (eq? (adt-value-variant *tesl-case-9) 'Ok)) (thsl-src! "example/learn/lesson46-result-type.tesl" 167 (list) (lambda () (raw-value "")))] [(and (adt-value? *tesl-case-9) (eq? (adt-value-variant *tesl-case-9) 'Err)) (let ([e (hash-ref (adt-value-fields *tesl-case-9) 'error)]) (thsl-src! "example/learn/lesson46-result-type.tesl" 168 (list (cons 'e e)) (lambda () *e)))])))))

(define/pow
  (isOkResult [r : (Result Integer String)])
  #:returns Boolean
  (thsl-src-control! "example/learn/lesson46-result-type.tesl" 171 (list (cons 'r *r)) (lambda () (let ([tesl-case-10 *r]) (cond [(and (adt-value? *tesl-case-10) (eq? (adt-value-variant *tesl-case-10) 'Ok)) (thsl-src! "example/learn/lesson46-result-type.tesl" 172 (list) (lambda () (raw-value #t)))] [(and (adt-value? *tesl-case-10) (eq? (adt-value-variant *tesl-case-10) 'Err)) (thsl-src! "example/learn/lesson46-result-type.tesl" 173 (list) (lambda () (raw-value #f)))])))))

(define/pow
  (isErrUserResult [r : (Result String UserError)])
  #:returns Boolean
  (thsl-src-control! "example/learn/lesson46-result-type.tesl" 176 (list (cons 'r *r)) (lambda () (let ([tesl-case-11 *r]) (cond [(and (adt-value? *tesl-case-11) (eq? (adt-value-variant *tesl-case-11) 'Ok)) (thsl-src! "example/learn/lesson46-result-type.tesl" 177 (list) (lambda () (raw-value #f)))] [(and (adt-value? *tesl-case-11) (eq? (adt-value-variant *tesl-case-11) 'Err)) (thsl-src! "example/learn/lesson46-result-type.tesl" 178 (list) (lambda () (raw-value #t)))])))))

(define/pow
  (isErrStrResult [r : (Result String String)])
  #:returns Boolean
  (thsl-src-control! "example/learn/lesson46-result-type.tesl" 181 (list (cons 'r *r)) (lambda () (let ([tesl-case-12 *r]) (cond [(and (adt-value? *tesl-case-12) (eq? (adt-value-variant *tesl-case-12) 'Ok)) (thsl-src! "example/learn/lesson46-result-type.tesl" 182 (list) (lambda () (raw-value #f)))] [(and (adt-value? *tesl-case-12) (eq? (adt-value-variant *tesl-case-12) 'Err)) (thsl-src! "example/learn/lesson46-result-type.tesl" 183 (list) (lambda () (raw-value #t)))])))))
