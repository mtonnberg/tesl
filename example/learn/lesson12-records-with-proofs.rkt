...
(define-checker
  ...
  (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 54 (list (cons 's *s)) (lambda () (if (and (tesl-gt? (raw-value (tesl_import_String_length *s)) 0) (tesl-le? (raw-value (tesl_import_String_length *s)) 120)) (accept (SafeTitle s) #:value *s) (reject "title must be 1-120 characters" #:http-code 400)))))

(define-checker
  (checkLength [s : String])
  #:returns [s : String ::: (TitleLength s)]
  (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 63 (list (cons 's *s)) (lambda () (if (tesl-le? (raw-value (tesl_import_String_length *s)) 500) (accept (TitleLength s) #:value *s) (reject "too long" #:http-code 400)))))
...
(define/pow
  ...
  #:returns SafeMessage
  (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 91 (list (cons 'title *title) (cons 'body *body)) (lambda () (SafeMessage #:title title #:body body))))

(define-checker
  (checkPositiveInt [n : Integer])
  #:returns [n : Integer ::: (IsPositive n)]
  (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 209 (list (cons 'n *n)) (lambda () (if (tesl-gt? *n 0) (accept (IsPositive n) #:value *n) (reject "must be positive" #:http-code 400)))))

(define-checker
  (checkPriceExceedsQuantity [price : Integer] [quantity : Integer])
  #:returns [price : Integer ::: (PriceExceedsQuantity price quantity)]
  (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 219 (list (cons 'price *price) (cons 'quantity *quantity)) (lambda () (if (tesl-gt? *price *quantity) (accept (PriceExceedsQuantity price quantity) #:value *price) (reject "price must exceed quantity" #:http-code 422)))))
...
(define/pow
  ...
  #:returns OrderLine
  (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 248 (list (cons 'price *price) (cons 'quantity *quantity) (cons 'recordProof *recordProof)) (lambda () (OrderLine #:price price #:quantity quantity))))
...
(define/pow
  ...
  #:returns String
  (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 263 (list (cons 'order *order)) (lambda () (format "order: price=~a, qty=~a" (tesl-display-val (tesl-dot/runtime order 'price 'OrderLine)) (tesl-display-val (tesl-dot/runtime order 'quantity 'OrderLine))))))

(module+ test
  (require rackunit)
  (test-case "checkSafeTitle valid"
    (call-with-fresh-memory-db '() (lambda ()
  (define s1 (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 125 (list) (lambda () "hello")))
  ...
  (check-equal? (raw-value (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 127 (list (cons 'x x) (cons 's1 s1)) (lambda () x))) "hello")
  (define s2 (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 128 (list (cons 'x x) (cons 's1 s1)) (lambda () "a")))
  ...
  (check-equal? (raw-value (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 130 (list (cons 'y y) (cons 's2 s2) (cons 'x x) (cons 's1 s1)) (lambda () y))) "a")
    ))
  )

  (test-case "checkSafeTitle rejects"
    (call-with-fresh-memory-db '() (lambda ()
  (let ([tesl-ef-result (with-handlers ([exn:fail? (lambda (e) 'tesl-exception)]) (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 134 (list) (lambda ()
                          (checkSafeTitle ""))))])
  ...
  (test-case "checkLength valid"
    (call-with-fresh-memory-db '() (lambda ()
  (define s1 (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 138 (list) (lambda () "")))
  ...
  (check-equal? (raw-value (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 140 (list (cons 'y y) (cons 's1 s1)) (lambda () y))) "")
  (define s2 (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 141 (list (cons 'y y) (cons 's1 s1)) (lambda () "hello")))
  ...
  (check-equal? (raw-value (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 143 (list (cons 'x x) (cons 's2 s2) (cons 'y y) (cons 's1 s1)) (lambda () x))) "hello")
    ))
  )

  (test-case "createMessage valid"
    (call-with-fresh-memory-db '() (lambda ()
  (define rawTitle (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 147 (list) (lambda () "My Title")))
  ...
  (define rawBody (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 149 (list (cons 't t) (cons 'rawTitle rawTitle)) (lambda () "Some body text")))
  ...
  (define msg (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 151 (list (cons 'b b) (cons 'rawBody rawBody) (cons 't t) (cons 'rawTitle rawTitle)) (lambda () (createMessage t b))))
  (check-equal? (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 152 (list (cons 'msg msg) (cons 'b b) (cons 'rawBody rawBody) (cons 't t) (cons 'rawTitle rawTitle)) (lambda () (raw-value (tesl-dot/runtime msg 'title 'SafeMessage)))) "My Title")
  (check-equal? (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 153 (list (cons 'msg msg) (cons 'b b) (cons 'rawBody rawBody) (cons 't t) (cons 'rawTitle rawTitle)) (lambda () (raw-value (tesl-dot/runtime msg 'body 'SafeMessage)))) "Some body text")
    ))
  )

  (test-case "valid OrderLine"
    (call-with-fresh-memory-db '() (lambda ()
  (define rawP (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 266 (list) (lambda () 10)))
  ...
  (define rawQ (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 268 (list (cons 'p p) (cons 'rawP rawP)) (lambda () 3)))
  ...
  (define order (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 271 (list (cons 'pq pq) (cons 'q q) (cons 'rawQ rawQ) (cons 'p p) (cons 'rawP rawP)) (lambda () (makeOrderLine p q (detach-all-proof pq)))))
  (define orderAlt (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 272 (list (cons 'order order) (cons 'pq pq) (cons 'q q) (cons 'rawQ rawQ) (cons 'p p) (cons 'rawP rawP)) (lambda () (attach-proof (OrderLine #:price p #:quantity q) (detach-all-proof pq)))))
  (check-equal? (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 273 (list (cons 'orderAlt orderAlt) (cons 'order order) (cons 'pq pq) (cons 'q q) (cons 'rawQ rawQ) (cons 'p p) (cons 'rawP rawP)) (lambda () (raw-value (tesl-dot/runtime order 'price 'OrderLine)))) 10)
  (check-equal? (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 274 (list (cons 'orderAlt orderAlt) (cons 'order order) (cons 'pq pq) (cons 'q q) (cons 'rawQ rawQ) (cons 'p p) (cons 'rawP rawP)) (lambda () (raw-value (tesl-dot/runtime order 'quantity 'OrderLine)))) 3)
    ))
  )

  (test-case "checkPriceExceedsQuantity rejects price <= quantity"
    (call-with-fresh-memory-db '() (lambda ()
  (define rawP (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 280 (list) (lambda () 3)))
  ...
  (define rawQ (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 282 (list (cons 'p p) (cons 'rawP rawP)) (lambda () 10)))
  ...
  (let ([tesl-ef-result (with-handlers ([exn:fail? (lambda (e) 'tesl-exception)]) (thsl-src! "example/learn/lesson12-records-with-proofs.tesl" 284 (list (cons 'q q) (cons 'rawQ rawQ) (cons 'p p) (cons 'rawP rawP)) (lambda ()
