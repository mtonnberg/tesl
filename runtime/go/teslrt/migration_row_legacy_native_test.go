...
func TestPgRowLegacyContractApp(t *testing.T) {
	root := os.Getenv("TESL_ROW_LEGACY_PROGRAMS")
	if root == "" {
		t.Skip("compiler-generated Legacy applications required")
	}
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("worker did not materialize preexisting Legacy rows", converted, provisional, err)
		}
		time.Sleep(20 * time.Millisecond)
	}
	stop()
	v2 := pgStartRowAccessApp(t, f, root, "v2")
	var oldText, author string
	if err := request.QueryRow(ctx, "select metadata->>'oldText',author from notes.notes where id='one'").Scan(&oldText, &author); err != nil || oldText != "one" || author != "former" {
		t.Fatal("worker must encode exact legacy JSONB and constant", oldText, author, err)
	}
	oldBody := call(v1, "GET", "/all", 200)
	if !strings.Contains(oldBody, `"oldText":"one"`) || !strings.Contains(oldBody, `"author":"former"`) {
		t.Fatal("original app cannot decode legacy columns", oldBody)
	}
	newBody := call(v2, "GET", "/all", 200)
	if strings.Contains(newBody, `"metadata"`) || strings.Contains(newBody, `"author"`) {
		t.Fatal("removed logical fields leaked into current rows", newBody)
	}
	call(v2, "POST", "/absent", 200)
	if err := request.QueryRow(ctx, "select metadata->>'oldText',author from notes.notes where id='absent'").Scan(&oldText, &author); err != nil || oldText != "absent" || author != "former" {
		t.Fatal("new row omitted old-compatible values", oldText, author, err)
	}
	call(v1, "GET", "/all", 200)
	call(v1, "PUT", "/one", 200)
	var marker int
	if err := request.QueryRow(ctx, "select _tesl_v from notes.notes where id='one'").Scan(&marker); err != nil || marker != 1 {
		t.Fatal("late original Legacy writer must invalidate", marker, err)
	}
	call(v2, "GET", "/all", 200)
	if err := request.QueryRow(ctx, "select _tesl_v from notes.notes where id='one'").Scan(&marker); err != nil || marker != 1 {
	...
	if err := request.QueryRow(ctx, "select metadata->>'oldText',author,_tesl_v from notes.notes where id='one'").Scan(&oldText, &author, &marker); err != nil || oldText != "edited" || author != "former" || marker != 2 {
		t.Fatal("final pass did not durably supply legacy values after late old write", oldText, author, marker, err)
	}
	...
	if err := request.QueryRow(ctx, "select metadata is null and author is null,_tesl_v from notes.notes where id='three'").Scan(&omitted, &marker); err != nil || !omitted || marker != 2 {
		t.Fatal("prepared Legacy insert must omit old storage and stamp current generation", omitted, marker, err)
	}
	call(v2, "PUT", "/one", 200)
	call(v2, "GET", "/all", 200)
	pause.resume()
	...
	if err := request.QueryRow(ctx, "select to_regclass('notes.legacy_author_idx') is not null").Scan(&oldIndexExists); err != nil || oldIndexExists {
		t.Fatal("Contract retained old-field index", oldIndexExists, err)
	}
	...
	if err := request.QueryRow(ctx, "select count(*) from information_schema.columns where table_schema='notes' and table_name='notes' and column_name in ('metadata','author')").Scan(&oldColumns); err != nil || oldColumns != 0 {
		t.Fatal("Contract did not remove exact old JSONB and primitive legacy storage", oldColumns, err)
	}
	...
	if strings.Contains(body, `"metadata"`) || strings.Contains(body, `"author"`) {
		t.Fatal("settled current rows contain removed fields", body)
	}
	call(v1, "GET", "/all", 503)
	call(v1, "PUT", "/one", 503)
	run("v2", "contract")
	t.Log("preexisting logical-field removal worker backfill, late old write, finality, pre-DDL current-only writes, same-process post-Contract queries and idempotent resume passed")
}
