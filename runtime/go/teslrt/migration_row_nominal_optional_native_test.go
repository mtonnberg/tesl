...
func TestPgRowNominalOptionalLifecycle(t *testing.T) {
	...
	for _, kind := range []string{"record", "adt"} {
		t.Run(kind, func(t *testing.T) {
			...
			checkOptional := func(column string) {
				t.Helper()
				var nulls, values int
				// Only fixed fixture column names reach this query.
				if column != "spare" && column != "alias" {
					t.Fatal("unexpected optional field", column)
				}
				if err := request.QueryRow(f.ctx, `select count(*) filter (where `+column+` is null), count(*) filter (where `+column+` is not null) from notes.notes`).Scan(&nulls, &values); err != nil || nulls != 1 || values != 3 {
					t.Fatal("optional SQL NULL or payload changed", column, nulls, values, err)
				}
			}
			checkOptional("spare")
			var original string
			...
			checkOptional("alias")
			t.Log("actual unchanged handlers retained optional JSONB fields and all nominal keys through worker cursor, late old write, final pass and Contract", original)
		})
	}
}
